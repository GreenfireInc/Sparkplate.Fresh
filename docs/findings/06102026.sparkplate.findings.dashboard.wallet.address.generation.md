...
- **Sparkplate** generates a **brand-new random mnemonic on every click**, derives the address **in the renderer** (`@/utils/cryptoGenerator`), always at **index 0 on mainnet**, and then **discards the seed/private key**, persisting only the public address.
