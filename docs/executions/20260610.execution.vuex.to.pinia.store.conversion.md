...
**Status:** Implemented — Phase 0 complete; first domain store (contacts) wired into the AddressBook view; **Phase 1 foundational stores complete (`useSettingsStore` + `useCoinsStore` + `useAccountsStore`)**; **real account creation & login wired on `useAccountsStore` (§9.7)**; address-only `useWalletsStore` landed and wired into `Dashboard.vue` (§8.7)
...
- **§9 implemented — real account creation & login.** Added `src/services/account/service.account.User.ts`
  (localStorage `sparkplate.accounts.users.v1`; PBKDF2/`crypto-js` password hashing; `createUser`/`verifyLogin`),
  promoted `useAccountsStore` to service-backed (`loadUsers`/`signup`/`authenticate`, `MOCK_USERS` removed),
  and wired the signup modal, `UserModal`, and `LoginStandard` to it. See §9.7 for the as-built notes.
  References renumbered to §10.
...
| `src/services/account/service.account.User.ts` | **New** — account credential service (PBKDF2 hash, `localStorage`; §9.7) |
| `src/components/authentication/registration/01.registration.signUp.vue` | `handleSignup` → `useAccountsStore().signup` (§9.7) |
| `src/components/authentication/user/UserModal.vue` | `handleSignIn` → `useAccountsStore().authenticate` (§9.7) |
| `src/components/authentication/loginStandard/LoginStandard.vue` | Lists real saved users from `useAccountsStore` (§9.7) |
...
> **Implemented** (June 10, 2026) — §9.1–9.6 below are the design; §9.7 records what actually shipped.
...
### 9.7 As-built (implemented June 10, 2026)

The §9.1–9.6 design shipped, with these concrete choices/deviations:

- **Account service — `src/services/account/service.account.User.ts`.** Implements `StoredUser` /
  `PublicUser` (hash + salt never leave the module), `createUser` (unique-email guard, salted hash,
  sanitize), `verifyLogin` (constant-time hash compare), `listUsers`, `getUserByEmail`, `removeUser`.
  Persists to `localStorage` key **`sparkplate.accounts.users.v1`** following the
  `service.addressBook.Contact.ts` pattern.
  - **Hashing: PBKDF2 via the already-installed `crypto-js`** (SHA-256, 100k iterations, 256-bit key,
    16-byte random salt) — chosen over `scrypt-js` because `crypto-js` is the project's established hasher
    (e.g. `ATOM.Cosmos.ts`, the cryptography tool tabs) and needs no async/worker plumbing. Either satisfies
    the design; this keeps it synchronous and convention-aligned.
- **`useAccountsStore` promoted to service-backed.** Added `loadUsers()`, async `signup(input)` (creates +
  auto-logs-in), and async `authenticate(email, password)` (returns `false` on bad creds). `all` now seeds
  **empty** and is filled from the service — the `MOCK_USERS` array was **removed** (a fresh install shows
  only "Create Account", which is the correct real-accounts behavior). `User` gained optional
  `firstName`/`lastName` (kept `name` so `tab.Settings.User.vue`'s `currentUser.name` split still works).
  `reset()` now clears only the in-memory **session** (accounts are service-owned, mirroring contacts §5).
  The store still enables **no** `persist`.
- **Component wiring.**
  - `01.registration.signUp.vue` — `handleSignup` is now `async` and calls `accounts.signup({...})`;
    on success it closes (auto-logged-in), on duplicate email it returns to the form and shows a
    `signupError`. The generated mnemonic is **not** stored on the account (§9.5).
  - `user/UserModal.vue` — `handleSignIn` is now `async` and calls `accounts.authenticate(userEmail, password)`,
    only closing on success and showing an "Invalid email or password." error otherwise (previously it
    ignored the password). Dropped its `useAuth` import.
  - `loginStandard/LoginStandard.vue` — lists real saved users via `storeToRefs(useAccountsStore()).all`
    and calls `loadUsers()` on mount (was `useAuth().mockUsers`).
- **`useAuth` shim unchanged** and still serves the read-only consumers (`App.vue`,
  `dropdown.authentication.vue`, `tab.Settings.User.vue`); `logout` there flows through the store.
- **Verification.** `vue-tsc --noEmit --skipLibCheck` — 0 errors in the new service, the store, and all
  five auth components/consumers.
- **Still deferred (unchanged from §9.6):** staying logged-in across restart (session intentionally not
  persisted), encrypted seed/secret custody, password reset, and MFA (`TwoFactorAuth.vue`) — all Phase 4.
...
- `src/services/account/service.account.User.ts` (account credentials, PBKDF2 via `crypto-js`); `00.references/from.Greenery/service/UserService.js` (V1 model)
