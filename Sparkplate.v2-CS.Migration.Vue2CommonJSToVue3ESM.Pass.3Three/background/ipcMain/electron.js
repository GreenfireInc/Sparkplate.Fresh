    ...
    win.webContents.on('did-finish-load', () => {
      ...
      if (options.type === 'print') {
        ...
        win.webContents
          ...
          .catch((err) => {
            reject(new Error(`Failed to save as PDF: ${err.message}`))
            win.destroy()
          })
      }
    })
...
ipcMain.handle('getBugTracking', async () => {
  // Implement the logic to get the bug tracking status
  // For example, you might return a status from a settings file or database
  const bugTrackingStatus = true // Replace with actual logic
  return { status: bugTrackingStatus }
})
