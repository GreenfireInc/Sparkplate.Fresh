...
export const sparkplateMetadata = {
  name: app.name,
...
export async function parseTxFromParams(params, provider) {
  const tx = { ...params[0] }
  tx.chainId = await provider.getNetwork().then((n) => n.chainId)
  if (!tx.gasLimit && tx.gas) tx.gasLimit = tx.gas
  tx.gasPrice = await provider.getGasPrice()
  // Remove gas prop to avoid ethers complaints
  delete tx.gas
  return tx
}

export async function signTx({ privateKey, tx, network, signer }) {
  if (!signer) signer = infura.getSigner({ privateKey, network })
