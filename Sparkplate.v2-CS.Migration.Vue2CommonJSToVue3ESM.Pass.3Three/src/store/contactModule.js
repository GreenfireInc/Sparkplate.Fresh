...
export async function doesContactCurrencyExist(currency, contactId) {
  const { coinTicker, address } = currency
  const coinWallets = await contactService.getContactWallets({
    contactId,
    coinTicker
  })

  return coinWallets.some((w) => w.address === address)
}
      ...
      for (const i in contacts) {
        if (contacts[i].id === contactId) {
          if (!state.list[i].wallets[coinTicker]) {
            Vue.set(state.list[i].wallets, coinTicker, [wallet])
          } else {
            const updatedList = [wallet, ...state.list[i].wallets[coinTicker]]
            Vue.set(state.list[i].wallets, coinTicker, updatedList)
          }
          break
        }
      }
