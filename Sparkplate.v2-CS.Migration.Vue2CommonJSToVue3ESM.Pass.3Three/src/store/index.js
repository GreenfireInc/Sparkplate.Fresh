...
import walletModule from './walletModule.js'
...
import web3ConnectionModule from './web3ConnectionModule.js'
import {
  getCoinsData,
  getCoinsMeta,
  priceConversions,
...
export const coinInfoPath = './src/assets/coininfo.json'
export const coinMetaPath = './src/assets/coinmeta.json'

export const ls = new SecureLS({ isCompression: false })
Vue.use(Vuex)

export function writeCoinData(filePath, filePayload) {
  return window.fs.writeFileSync(filePath, filePayload, 'utf8')
}

export async function loadParseJSON(file) {
