import { generateStandardFilenameWithCurrency } from "./filenameStructure";
...
export async function captureExportCanvas(
  ...
  // Display full wallet address with proper centered wrapping
  ...
  // Draw Address and Public Key side by side with currency ticker prefix
  y = drawSideBySideSections(
    ctx,
    "Address:",
    `${currencyTicker}://${crypto.address}`,
    false,
    "Public Key:",
    `${currencyTicker}://${crypto.cryptoPublicKey}`,
    false,
    PADDING,
    y,
    contentWidth
...
export async function exportCryptoAsPNG(crypto: ExportCryptoAddress) {
  ...
  link.download = generateStandardFilenameWithCurrency(crypto.currency, "png");
...
export async function exportCryptoAsPDF(crypto: ExportCryptoAddress) {
  ...
  pdf.save(generateStandardFilenameWithCurrency(crypto.currency, "pdf"));
}
