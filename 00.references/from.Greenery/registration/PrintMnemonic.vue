  ...
  methods: {
    ...
    async initPrint() {
      ...
      const { response } = await this.$dialog.showMessageBox({
        message:
          'Please keep this printout safe and secure. Store it behind a picture frame, in a safety deposit box or in between your mattress.',
        type: 'warning',
    ...
    async printCanvas() {
      ...
      try {
        await this.$exportAsPDF('mnemonicPhrase.pdf', element, {
          type: 'print',
          settings: { pageSize: 'Letter', landscape: true }
        })
        this.$toast.success('Document sent to printer.', 'Success')
      } catch (err) {
