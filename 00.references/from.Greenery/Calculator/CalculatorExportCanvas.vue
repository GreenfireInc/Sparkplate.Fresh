<template>
  <canvas ref="canvasContainer" :width="width" :height="height" hidden></canvas>
</template>
...
export default {
  ...
  methods: {
    ...
    async drawBackground() {
      const bgPath = this.canvasBgPath
    ...
    async drawText() {
      ...
      const rateFormatted = this.solution.rate
      const rate = `1 ${from.symbol} ≈ ${rateFormatted} ${to.symbol}`
      this.ctx.fillText(rate, this.canvasCenter, 620)

      const amountFormatted = parseFloat(this.args.amount).toLocaleString(
        'en-US',
        localeStringOpts
      )
      const solutionFormatted = this.solution.amount
      const conversion = `${amountFormatted} ${from.symbol} ≈ ${solutionFormatted} ${to.symbol}`
      this.ctx.fillText(conversion, this.canvasCenter, 740)
    },
    async drawSymbols() {
      const { from, to } = this.args
      const posX = this.canvasCenter + 50
      const baseUrl = './assets/cryptoicons/'
      // draw from symbol
      const fromPath = `${baseUrl}/${from.symbol.toLowerCase()}.svg`
      ...
      // draw to symbol
      const toPath = `${baseUrl}/${to.symbol.toLowerCase()}.svg`
      await this.drawImage(
        toPath,
        posX,
        360 - 7,
