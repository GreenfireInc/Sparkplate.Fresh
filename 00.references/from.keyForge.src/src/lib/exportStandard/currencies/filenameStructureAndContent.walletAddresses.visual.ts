 ...
 *   } from "@/lib/exportStandard/currencies/filenameStructureAndContent.walletAddresses.visual";
 * 
 *   import { GeneralAddress } from "@/lib/exportStandard/currencies/filenameStructureAndContent.walletAddresses.text";
 * 
...
export async function captureGeneralExportCanvas(
  ...
  if (rootGPGFingerprint) {
    ...
    ctx.fillStyle = "#666666";
    ctx.textAlign = "center";
    ctx.fillText("Key Fingerprint:", EXPORT_WIDTH_PX / 2, y + 45);
    ctx.font = "bold 11px monospace";
    ctx.fillStyle = "#333333";
    ctx.fillText(rootGPGFingerprint, EXPORT_WIDTH_PX / 2, y + 62);
    y += 90;
  } else {
    y += 80;
  }
...
export async function exportGeneralAddressesAsPNG(
  ...
  const canvas = await captureGeneralExportCanvas(addresses, rootGPGFingerprint, { scale: 2 });
...
export async function exportGeneralAddressesAsPDF(
  ...
  const pxToMmX = (px: number) => {
    const normalizedX = px / EXPORT_WIDTH_PX;
    if (imgHeight > pageHeight) {
  ...
  const pxToMmY = (py: number) => {
    const normalizedY = py / canvasLogicalHeight;
    if (imgHeight > pageHeight) {
  ...
  const cardWidth = (EXPORT_WIDTH_PX - PADDING * 2 - CARD_SPACING) / CARDS_PER_ROW;
  const cardHeight = QR_SIZE_PX + 115;
  const rows = Math.ceil(addresses.length / CARDS_PER_ROW);
  // Y offset matches canvas: +90 if we have root GPG fingerprint, +80 otherwise
  const y = PADDING + (rootGPGFingerprint ? 90 : 80);

  // Add text for each address card
  for (let i = 0; i < addresses.length; i++) {
    const address = addresses[i];
    const row = Math.floor(i / CARDS_PER_ROW);
    const col = i % CARDS_PER_ROW;
    ...
    // Get currency ticker and address with prefix
    const currencyTicker = address.currency.toLowerCase();
    const addressWithPrefix = `${currencyTicker}://${address.address}`;
    
    // Derivation path (above QR code)
    addText(address.derivationPath, cardX + cardWidth / 2, cardY, 10, 'center');
    
    // Address (below QR code)
    const qrY = cardY + 20;
    const addressY = qrY + QR_SIZE_PX + 25;
    addText(addressWithPrefix, cardX + cardWidth / 2, addressY, 9, 'center');
    
    // GPG fingerprint (below address)
    const fingerprintY = addressY + 20;
    addText(`GPG: ${address.keyFingerprint}`, cardX + cardWidth / 2, fingerprintY, 8, 'center');
  }
