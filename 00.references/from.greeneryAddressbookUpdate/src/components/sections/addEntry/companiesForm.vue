      ...
      <div class="tab-content">
        <div class="tab-panel" :class="{ 'active-panel': currentTab === 'general' }">
          <div class="form-grid">
            <div class="form-group">
              ...
              <input type="text" id="company-name" v-model="companyForm.name" placeholder="The name of the Company" required/>
            </div>
            <div class="form-group">
              <label for="company-email">Email address</label>
              <input type="email" id="company-email" v-model="companyForm.email" placeholder="Main Email address associated with it" />
            </div>
            <div class="form-group">
              <label for="company-contact">Contact</label>
              <input type="text" id="company-contact" v-model="companyForm.contact" placeholder="Contact person" />
            </div>
            <div class="form-group">
              <label for="company-contact-email">Contact Email</label>
              <input type="email" id="company-contact-email" v-model="companyForm.contactEmail" placeholder="Contact person email address" />
            </div>
          </div>
        </div>
        ...
        <div class="tab-panel" :class="{ 'active-panel': currentTab === 'advanced' }">
          <div class="wallets-section">
              <div v-if="companyWallets.length > 0" class="wallet-group wallet-header">
                  <strong>Coin</strong>
                  <strong>Address</strong>
                  <span></span> <!-- Empty span for alignment -->
              </div>
              <div v-for="(wallet, index) in companyWallets" :key="index" class="wallet-group">
...
const companyWallets = ref<Partial<Wallet>[]>([]);

function addWalletRow() {
  companyWallets.value.push({ coinTicker: '', address: '' });
}

function removeWallet(index: number) {
  companyWallets.value.splice(index, 1);
}

function handleSave() {
  emit('saved', {
    form: companyForm.value,
    wallets: companyWallets.value
  });
}
...
const handleFileImportFromModal = async (file: File) => {
  try {
    ...
    // Add imported wallets to the existing company wallets array
    companyWallets.value.push(...result.wallets);
  } catch (error) {
