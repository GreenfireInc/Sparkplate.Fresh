...
import WalletImportConfirmModal from './confirmations/WalletImportConfirmModal.vue';
import { parseWalletJsonFile, type ImportedWallet } from '../../lib/cores/importStandard/importWallet.json';
import { addWallet, getWalletsForContact } from '../../services/walletService';
...
const handleConfirmImport = async (wallets: ImportedWallet[]) => {
  try {
    // Get existing wallets for this contact to check for duplicates
    const existingWallets = await getWalletsForContact(props.contactId);
    
    // Helper function to check if a wallet already exists
    const walletExists = (coinTicker: string, address: string): boolean => {
      return existingWallets.some(
        w => w.coinTicker === coinTicker && w.address === address
      );
    };
    
    // Filter out duplicate wallets and add only new ones
    const newWallets = wallets.filter(
      wallet => !walletExists(wallet.coinTicker, wallet.address)
    );
    const duplicateCount = wallets.length - newWallets.length;
    
    // Add only new wallets to the database
    ...
    // Close import modal and reset form first
    closeImportModal();
    resetForm();
    
    // Emit event to notify parent that wallets were imported (for refresh)
    // Do this BEFORE showing alerts and closing the modal to ensure parent processes it
    if (newWallets.length > 0) {
      console.log('Emitting wallets-imported event for', newWallets.length, 'wallets');
      emits('wallets-imported');
      
      // Give the parent component time to process the event before showing alerts
      await new Promise(resolve => setTimeout(resolve, 100));
    }
    
    // Provide feedback to user about import results
    if (duplicateCount > 0) {
      const message = newWallets.length > 0
        ? `Imported ${newWallets.length} wallet(s). ${duplicateCount} duplicate(s) skipped.`
        : `All ${duplicateCount} wallet(s) were duplicates and skipped.`;
      alert(message);
    } else if (newWallets.length > 0) {
      alert(`Successfully imported ${newWallets.length} wallet(s).`);
    }
