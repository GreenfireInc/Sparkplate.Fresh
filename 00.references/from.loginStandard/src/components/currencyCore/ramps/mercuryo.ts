// Mercuryo Ramp API Integration
// Global crypto payment platform

export interface MercuryoConfig {
  apiKey?: string;
  baseUrl?: string;
  sandbox?: boolean;
}

export class MercuryoRampClass {
  private config: MercuryoConfig;
  private baseUrl: string;

  constructor(config: MercuryoConfig = {}) {
    this.config = config;
    this.baseUrl = config.baseUrl || (config.sandbox ? 'https://api-sandbox.mercuryo.io' : 'https://api.mercuryo.io');
  }
  ...
  static readonly info = {
    ...
    apiDocs: 'https://mercuryo.io/api/',
    status: 'Active',
    supportedCurrencies: '50+',
    features: ['Credit Card', 'Bank Transfer', 'Apple Pay', 'Google Pay', 'Global Coverage'],
  };

  // Social Media Links
  static readonly socialMedia = {
    twitter: 'https://twitter.com/mercuryo_io',
    telegram: 'https://t.me/mercuryo',
    reddit: 'https://www.reddit.com/r/mercuryo/',
    youtube: 'https://www.youtube.com/mercuryo',
    facebook: 'https://www.facebook.com/mercuryo',
    linkedin: 'https://www.linkedin.com/company/mercuryo',
  };

  /**
   * Make request to Mercuryo API
   */
  private async makeRequest(endpoint: string, params: Record<string, any> = {}): Promise<any> {
    ...
    if (!response.ok) {
      throw new Error(`Mercuryo API error: ${response.status} ${response.statusText}`);
    }

    const result = await response.json();
    if (result.error) {
      throw new Error(`Mercuryo API error: ${result.error}`);
    }
...
export const MercuryoRamp = {
  // Quick access methods
  async getCurrencies(): Promise<any[]> {
    const instance = new MercuryoRampClass();
  ...
  // Static properties
  info: MercuryoRampClass.info,
  socialMedia: MercuryoRampClass.socialMedia,
};

// Export the class as default
export default MercuryoRamp;
