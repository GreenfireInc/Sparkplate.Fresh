// AlchemyPay Ramp API Integration
// Global payment infrastructure

export interface AlchemyPayConfig {
  apiKey?: string;
  baseUrl?: string;
  sandbox?: boolean;
}

export class AlchemyPayRampClass {
  private config: AlchemyPayConfig;
  private baseUrl: string;

  constructor(config: AlchemyPayConfig = {}) {
    this.config = config;
    this.baseUrl = config.baseUrl || (config.sandbox ? 'https://api-sandbox.alchemypay.org' : 'https://api.alchemypay.org');
  }
  ...
  static readonly info = {
    ...
    apiDocs: 'https://docs.alchemypay.org/',
    status: 'Active',
  ...
  static readonly socialMedia = {
    ...
    reddit: 'https://www.reddit.com/r/alchemypay/',
    youtube: 'https://www.youtube.com/alchemypay',
    facebook: 'https://www.facebook.com/alchemypay',
    linkedin: 'https://www.linkedin.com/company/alchemypay',
  };

  /**
   * Make request to AlchemyPay API
   */
  private async makeRequest(endpoint: string, params: Record<string, any> = {}): Promise<any> {
    ...
    if (!response.ok) {
      throw new Error(`AlchemyPay API error: ${response.status} ${response.statusText}`);
    }

    const result = await response.json();
    if (result.error) {
      throw new Error(`AlchemyPay API error: ${result.error}`);
    }
...
export const AlchemyPayRamp = {
  // Quick access methods
  async getCurrencies(): Promise<any[]> {
    const instance = new AlchemyPayRampClass();
  ...
  // Static properties
  info: AlchemyPayRampClass.info,
  socialMedia: AlchemyPayRampClass.socialMedia,
};

// Export the class as default
export default AlchemyPayRamp;
