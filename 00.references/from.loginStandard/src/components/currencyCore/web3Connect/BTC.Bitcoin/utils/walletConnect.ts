/**
 * Bitcoin WalletConnect Integration
 * 
 * Handles WalletConnect v2 protocol for Bitcoin dApp connections.
 * Bitcoin uses its own namespace in WalletConnect.
 */

import { Core } from '@walletconnect/core';
import { Web3Wallet } from '@walletconnect/web3wallet';
import { buildApprovedNamespaces, getSdkError } from '@walletconnect/utils';
import { btcMethods } from './btcRPCMethods';

// App metadata for WalletConnect
const appMetadata = {
  name: 'LoginStandard',
  description: 'LoginStandard - Bitcoin Wallet',
...
class BTCWalletConnect {
  ...
  constructor() {
    this.requestTypes = {
      transactionRequests: ['btc_signAndSendPSBT', 'btc_signPSBT', 'btc_createPSBT'],
      queryRequests: ['btc_getUTXOs', 'btc_getBalance', 'btc_getTransaction'],
      signMessageRequests: ['btc_signMessage', 'btc_verifyMessage'],
      sessionRequest: 'session_request',
      sessionProposal: 'session_proposal',
      otherRequests: ['btc_sendRawTransaction', 'btc_getBlock']
    };
  }
  ...
  async handleSessionProposal({ 
    ...
    btcChains.forEach((chain) => {
      wallets.forEach((wallet) => {
        const btcAccountString = `${chain}:${wallet.address}`;
        accounts.push(btcAccountString);
      });
    });
    ...
    const approvedNamespaces = buildApprovedNamespaces({
      ...
      supportedNamespaces: {
        bitcoin: {
          chains: btcChains,
          methods: Object.keys(btcMethods),
  ...
  async handleSessionRequest({ 
    ...
    if (!approved) {
      ...
      const result = await btcMethods[method as keyof typeof btcMethods]({
        wallet,
        params,
  ...
  getAddressFromRequestParams(method: string, params: any[]): string | undefined {
    ...
    if (requestTypes.transactionRequests.includes(method)) {
      ...
      return method === 'btc_signMessage' ? params[1] : params[0];
    }
  }
}

export default new BTCWalletConnect();
