/**
 * Arweave WalletConnect Integration
 * 
 * Handles WalletConnect v2 protocol for Arweave dApp connections.
 * Note: Arweave primarily uses ArConnect, but WalletConnect support may be available.
 */

import { Core } from '@walletconnect/core';
import { Web3Wallet } from '@walletconnect/web3wallet';
import { buildApprovedNamespaces, getSdkError } from '@walletconnect/utils';
import { arweaveMethods } from './arweaveRPCMethods';

// App metadata for WalletConnect
const appMetadata = {
  name: 'LoginStandard',
  description: 'LoginStandard - Arweave Wallet',
...
class ArweaveWalletConnect {
  ...
  constructor() {
    this.requestTypes = {
      transactionRequests: ['arweave_sendTransaction', 'arweave_createTransaction', 'arweave_signTransaction'],
      sessionRequest: 'session_request',
      sessionProposal: 'session_proposal',
      otherRequests: ['arweave_getAddress', 'arweave_getBalance', 'arweave_getTransaction']
    };
  }
  ...
  async handleSessionProposal({ 
    ...
    // Build Arweave accounts
    const arweaveChains = [
      'arweave:1' // Mainnet
    ];

    const accounts: string[] = [];
    arweaveChains.forEach((chain) => {
      wallets.forEach((wallet) => {
        const arweaveAccountString = `${chain}:${wallet.address}`;
        accounts.push(arweaveAccountString);
      });
    });
    ...
    const approvedNamespaces = buildApprovedNamespaces({
      ...
      supportedNamespaces: {
        arweave: {
          chains: arweaveChains,
          methods: Object.keys(arweaveMethods),
  ...
  async handleSessionRequest({ 
    ...
    if (!approved) {
      ...
      const result = await arweaveMethods[method as keyof typeof arweaveMethods]({
        wallet,
        params,
        network,
  ...
  getAddressFromRequestParams(method: string, params: any[]): string | undefined {
    ...
    if (requestTypes.transactionRequests.includes(method)) {
      return params[0]?.from || params[0]?.owner;
    }
  }
}

export default new ArweaveWalletConnect();
