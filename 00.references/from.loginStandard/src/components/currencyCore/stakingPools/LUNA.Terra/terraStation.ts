// Terra Station Validators for Terra (LUNA)
// Native staking through Terra Station with 130+ validators

export const TerraStationValidators = {
  ...
  // API Information
  api: {
    baseUrl: "https://phoenix-lcd.terra.dev",
...
export async function getDelegations(delegatorAddress: string) {
  try {
    const response = await fetch(`https://phoenix-lcd.terra.dev/cosmos/staking/v1beta1/delegations/${delegatorAddress}`);
    if (response.ok) {
      const data = await response.json();
...
export async function getStakingRewards(delegatorAddress: string) {
  try {
    const response = await fetch(`https://phoenix-lcd.terra.dev/cosmos/distribution/v1beta1/delegators/${delegatorAddress}/rewards`);
    if (response.ok) {
      const data = await response.json();
      return data.rewards;
    }
...
// Helper function to get LUNA price from Terra oracle
export async function getLUNAPrice() {
  try {
    const response = await fetch('https://phoenix-lcd.terra.dev/terra/oracle/v1beta1/denoms/exchange_rates');
    if (response.ok) {
      const data = await response.json();
...
export default TerraStationValidators;
