// X2Y2 Marketplace Information
// NFT marketplace on Ethereum
// Source: Research compiled from multiple sources

export interface X2Y2Listing {
  tokenId: string;
...
export interface X2Y2Collection {
  contractAddress: string;
  name: string;
  description?: string;
  creator: string;
...
export const x2y2Marketplace = {
  name: "X2Y2",
  blockchain: "Ethereum",
  type: "NFT Marketplace",
  description: "NFT marketplace on Ethereum. Community-driven NFT marketplace with low fees, token rewards, and marketplace aggregation.",
  ...
  api: {
    endpoints: {
      mainnet: "https://api.x2y2.io/v1/",
      base: "https://api.x2y2.io/v1/",
    },
    pricing: {
    ...
    documentation: "https://docs.x2y2.io/api",
    rateLimit: "Rate limits apply (check documentation for current limits)",
    requiresApiKey: false,
    authentication: {
      type: "Optional API Key for higher limits",
      headerName: "X-API-Key",
      getApiKey: "https://x2y2.io/api",
    },
  },
  
  sdk: {
    npm: null,
    documentation: "https://docs.x2y2.io/developers",
...
async function getNFTPricing(contractAddress: string, tokenId: string) {
  const response = await fetch(\`https://api.x2y2.io/v1/nft/\${contractAddress}/\${tokenId}/price\`);
  const pricing: X2Y2Pricing = await response.json();
  return pricing;
}

// Get collection floor price
async function getCollectionFloorPrice(contractAddress: string) {
  const response = await fetch(\`https://api.x2y2.io/v1/collections/\${contractAddress}/floor-price\`);
...
async function searchNFTs(keyword: string) {
  const response = await fetch(\`https://api.x2y2.io/v1/nfts/search?keyword=\${encodeURIComponent(keyword)}\`);
  const data = await response.json();
  return data.results;
}
    `,
  ...
  socialMedia: {
    twitter: "https://twitter.com/x2y2",
    discord: "https://discord.gg/x2y2",
    telegram: "https://t.me/x2y2",
    medium: null,
    github: "https://github.com/x2y2",
  },
  ...
  notes: [
    ...
    "Low fees and marketplace aggregation",
  ],
};

export default x2y2Marketplace;
