// Terra Classic NFT Minting via Pinata IPFS
// Uploads files to Pinata IPFS, then creates CW721 NFT on Terra Classic
...
export class TerraClassicPinataNFTMinter {
  ...
  constructor(pinataConfig: PinataConfig, rpcEndpoint: string = 'https://terra-classic-lcd.publicnode.com', network: 'mainnet' | 'testnet' = 'mainnet') {
  ...
  async mintNFT(mnemonic: string, contractAddress: string, fileData: Buffer, metadata: Omit<NFTMetadata, 'image'>, tokenId: string, options: FileUploadOptions = {}): Promise<MintResult> {
    console.log('Starting Terra Classic NFT minting process...');
    ...
    const metadataUpload = await this.uploadMetadataToPinata(completeMetadata, { pinataMetadata: { name: `${metadata.name} - Metadata`, keyvalues: { type: 'nft-metadata', standard: 'CW721', chain: 'terra-classic', nftName: metadata.name } } });
    ...
    console.log(`Terra Classic NFT minted: Token ID ${terraClassicNFT.tokenId}`);
    ...
    return {
      ...
      tokenId: terraClassicNFT.tokenId,
      contractAddress,
      txHash: terraClassicNFT.txHash,
      finderUrl: `${explorerBase}/${terraClassicNFT.txHash}`,
    };
  }
  ...
  async mintMediaNFT(mnemonic: string, contractAddress: string, mediaData: Buffer, thumbnailData: Buffer, metadata: Omit<NFTMetadata, 'image' | 'animation_url'>, tokenId: string, options: { mediaOptions?: FileUploadOptions; thumbnailOptions?: FileUploadOptions } = {}): Promise<MintResult> {
    ...
    const metadataUpload = await this.uploadMetadataToPinata(completeMetadata, { pinataMetadata: { name: `${metadata.name} - Metadata`, keyvalues: { type: 'media-nft-metadata', standard: 'CW721', chain: 'terra-classic', nftName: metadata.name } } });
    const terraClassicNFT = await this.mintTerraClassicNFT(mnemonic, contractAddress, `ipfs://${metadataUpload.cid}`, completeMetadata, tokenId);
    const explorerBase = this.network === 'mainnet' ? 'https://finder.terra.money/classic/tx' : 'https://finder.terra.money/testnet/tx';
  ...
  async checkBalance(mnemonicOrAddress: string): Promise<{ address: string; balanceLunc: string }> {
    ...
    const balanceLunc = (parseInt(balance.amount) / 1000000).toString();

    return { address, balanceLunc };
  }
...
export default TerraClassicPinataNFTMinter;
