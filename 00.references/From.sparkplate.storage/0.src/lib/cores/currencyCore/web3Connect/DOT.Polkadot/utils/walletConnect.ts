/**
 * Polkadot WalletConnect Integration
 * 
 * Handles WalletConnect v2 protocol for Polkadot dApp connections.
...
import { dotMethods } from './dotRPCMethods';

// App metadata for WalletConnect
const appMetadata = {
  name: 'LoginStandard',
  description: 'LoginStandard - Polkadot Wallet',
...
class PolkadotWalletConnect {
  ...
  constructor() {
    this.requestTypes = {
      transactionRequests: ['dot_sendTransaction', 'dot_signAndSend', 'dot_signTransaction'],
      sessionRequest: 'session_request',
      sessionProposal: 'session_proposal',
      otherRequests: ['dot_getAccountInfo', 'dot_getBalance', 'dot_getTransaction']
    };
  }
  ...
  async handleSessionProposal({ 
    ...
    // Build Polkadot accounts for multiple chains
    const polkadotChains = [
      'polkadot:91b171bb158e2d3848fa23a9f1c25182', // Polkadot Mainnet
      'polkadot:b0a8d493285c2df73290dfb7e61f870f17b41801197a149ca93654499ea3daf9', // Kusama
      'polkadot:e143f23803ac50e8f6f8e62695d1ce9e4e1d68aa36c1cd2cfd15340213f3423e' // Westend Testnet
    ];

    const accounts: string[] = [];
    polkadotChains.forEach((chain) => {
      wallets.forEach((wallet) => {
        const polkadotAccountString = `${chain}:${wallet.address}`;
        accounts.push(polkadotAccountString);
      });
    });
    ...
    const approvedNamespaces = buildApprovedNamespaces({
      ...
      supportedNamespaces: {
        polkadot: {
          ...
          methods: Object.keys(dotMethods),
  ...
  async handleSessionRequest({ 
    ...
    if (!approved) {
      ...
      if (chainId.includes('kusama')) {
        network = 'kusama';
      } else if (chainId.includes('westend')) {
        network = 'testnet';
      }
      
      const result = await dotMethods[method as keyof typeof dotMethods]({
        wallet,
        params,
        network,
  ...
  getAddressFromRequestParams(method: string, params: any[]): string | undefined {
    ...
    if (requestTypes.transactionRequests.includes(method)) {
      return params[0]?.from || params[0]?.address;
    }
  }
}

export default new PolkadotWalletConnect();
