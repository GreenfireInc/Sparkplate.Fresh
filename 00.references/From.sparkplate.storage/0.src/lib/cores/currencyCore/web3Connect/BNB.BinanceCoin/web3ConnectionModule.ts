/**
 * BNB Smart Chain Web3 Connection Module
 * 
 * Vuex/Pinia module for managing BSC web3 connections
  ...
  actions: {
    ...
    async handleWCSessionRequest({ rootGetters, dispatch }: any, { approved, request }: any) {
      ...
      if (approved) {
        ...
        if (address) {
          wallet = rootGetters['wallets/getWalletByAddress']('bnb', address);
        }
      }
    ...
    async metamaskSwitchToBSC(_: any, { network }: { network: 'mainnet' | 'testnet' }) {
      return await metamaskConnect.switchToBSC(network);
    },
    ...
    async metamaskIsOnBSC(_: any) {
      return await metamaskConnect.isOnBSC();
    },
