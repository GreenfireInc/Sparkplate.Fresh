/**
 * Solana WalletConnect Integration
 * 
 * Handles WalletConnect v2 protocol for Solana dApp connections.
 * Solana uses its own namespace in WalletConnect.
 */

import { Core } from '@walletconnect/core';
import { Web3Wallet } from '@walletconnect/web3wallet';
import { buildApprovedNamespaces, getSdkError } from '@walletconnect/utils';
import { solMethods } from './solRPCMethods';

// App metadata for WalletConnect
const appMetadata = {
  name: 'LoginStandard',
  description: 'LoginStandard - Solana Wallet',
...
class SOLWalletConnect {
  ...
  constructor() {
    this.requestTypes = {
      transactionRequests: ['solana_signAndSendTransaction', 'solana_signTransaction'],
      signMessageRequests: ['solana_signMessage'],
      sessionRequest: 'session_request',
      sessionProposal: 'session_proposal',
      otherRequests: ['solana_sendTransaction', 'solana_getBalance', 'solana_getAccountInfo']
    };
  }
  ...
  async handleSessionProposal({ 
    ...
    solChains.forEach((chain) => {
      wallets.forEach((wallet) => {
        const solAccountString = `${chain}:${wallet.address}`;
        accounts.push(solAccountString);
      });
    });
    ...
    const approvedNamespaces = buildApprovedNamespaces({
      ...
      supportedNamespaces: {
        solana: {
          chains: solChains,
          methods: Object.keys(solMethods),
  ...
  async handleSessionRequest({ 
    ...
    if (!approved) {
      ...
      const result = await solMethods[method as keyof typeof solMethods]({
        wallet,
        params,
  ...
  getAddressFromRequestParams(method: string, params: any[]): string | undefined {
    ...
    if (requestTypes.transactionRequests.includes(method)) {
      return params[0]?.from || params[0]?.feePayer;
...
export default new SOLWalletConnect();
