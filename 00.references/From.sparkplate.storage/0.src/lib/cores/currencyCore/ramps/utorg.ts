// Utorg Ramp API Integration
// Global crypto payment platform

export interface UtorgConfig {
  apiKey?: string;
  baseUrl?: string;
  sandbox?: boolean;
}

export class UtorgRampClass {
  private config: UtorgConfig;
  private baseUrl: string;

  constructor(config: UtorgConfig = {}) {
    this.config = config;
    this.baseUrl = config.baseUrl || (config.sandbox ? 'https://api-sandbox.utorg.pro' : 'https://api.utorg.pro');
  }
  ...
  static readonly info = {
    ...
    apiDocs: 'https://utorg.pro/api/',
    status: 'Active',
    supportedCurrencies: '50+',
    features: ['Credit Card', 'Bank Transfer', 'Mobile Payment', 'Global Coverage'],
  };

  // Social Media Links
  static readonly socialMedia = {
    twitter: 'https://twitter.com/utorg',
    telegram: 'https://t.me/utorg',
    reddit: 'https://www.reddit.com/r/utorg/',
    youtube: 'https://www.youtube.com/utorg',
    facebook: 'https://www.facebook.com/utorg',
    linkedin: 'https://www.linkedin.com/company/utorg',
  };

  /**
   * Make request to Utorg API
   */
  private async makeRequest(endpoint: string, params: Record<string, any> = {}): Promise<any> {
    ...
    if (!response.ok) {
      throw new Error(`Utorg API error: ${response.status} ${response.statusText}`);
    }

    const result = await response.json();
    if (result.error) {
      throw new Error(`Utorg API error: ${result.error}`);
    }
...
export const UtorgRamp = {
  // Quick access methods
  async getCurrencies(): Promise<any[]> {
    const instance = new UtorgRampClass();
  ...
  // Static properties
  info: UtorgRampClass.info,
  socialMedia: UtorgRampClass.socialMedia,
};

// Export the class as default
export default UtorgRamp;
