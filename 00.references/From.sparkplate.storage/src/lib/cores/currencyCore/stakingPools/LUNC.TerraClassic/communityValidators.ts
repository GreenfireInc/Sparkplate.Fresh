// Terra Classic Community Validators for LUNC
// Community-run validators supporting Terra Classic revival

export const CommunityValidators = {
  name: "Terra Classic Community Validators",
  type: "Community Validators",
  website: "https://station.terrarebels.net/",
  ...
  fees: "Variable commission (typically 5-15%)",
  ...
  social: {
    ...
    discord: "https://discord.gg/terrarebels",
    telegram: "https://t.me/terrarebels",
  ...
  risks: [
    ...
    "Community validator reliability",
  ],
...
async function getCommunityValidators() {
  const validators = await terra.staking.validators();
  // Filter for community validators (you might need to maintain a list)
  const communityValidators = validators.filter(v => 
    v.description.moniker.includes('Rebel') || 
    v.description.moniker.includes('Community') ||
    v.description.moniker.includes('Classic')
  );
  console.log('Community validators:', communityValidators);
...
async function getValidatorDetails(validatorAddress: string) {
  const validator = await terra.staking.validator(validatorAddress);
  console.log('Validator details:', validator);
  return validator;
}

getValidatorDetails('terravaloper1...');
    `,
...
// Helper function to get community validators
export async function getCommunityValidators() {
  try {
    const response = await fetch('https://columbus-lcd.terra.dev/cosmos/staking/v1beta1/validators');
    if (response.ok) {
      const data = await response.json();
      // Filter for community validators (you might need to maintain a list)
      return data.validators.filter((validator: any) => 
        validator.description.moniker.includes('Rebel') || 
        validator.description.moniker.includes('Community') ||
        validator.description.moniker.includes('Classic') ||
        validator.description.moniker.includes('Terra') ||
        validator.description.moniker.includes('LUNC')
      );
    }
  } catch (error) {
    console.error('Error fetching community validators:', error);
  }
  return [];
}
