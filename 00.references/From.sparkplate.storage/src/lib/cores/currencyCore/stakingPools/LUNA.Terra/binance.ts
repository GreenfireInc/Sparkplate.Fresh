// Binance Staking Pool for Terra (LUNA)
...
client.stakingProductList({ product: 'STAKING' })
  .then(products => {
    const lunaProducts = products.filter(p => p.asset === 'LUNA');
    console.log('LUNA staking products:', lunaProducts);
  })
...
const stakeData = {
  product: 'STAKING',
  productId: 'LUNA*30', // 30-day locked staking
  amount: '1.0' // Amount in LUNA
};
...
client.stakingRecord({ product: 'STAKING' })
  .then(records => {
    const lunaRecords = records.filter(r => r.asset === 'LUNA');
    console.log('LUNA staking history:', lunaRecords);
  })
  .catch(err => console.error('Error:', err));
    `,
  },
...
export async function getBinanceStakingProducts() {
  try {
    ...
    if (response.ok) {
      ...
      return data.filter(product => product.asset === 'LUNA');
    }
  } catch (error) {
...
// Helper function to get LUNA price from Binance
export async function getLUNAPrice() {
  try {
    const response = await fetch('https://api.binance.com/api/v3/ticker/price?symbol=LUNAUSDT');
    if (response.ok) {
      const data = await response.json();
