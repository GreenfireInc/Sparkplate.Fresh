  ...
  decryptKeystore: async (keystore: unknown, password: string): Promise<string> => {
    try {
      ...
      if (tezosKeystore.kdf === 'Argon2') {
        // libsodium-wrappers-sumo's ESM build exposes its `crypto_*` functions
        // only on the default export object (named exports cover formatters
        // like `ready` / `to_hex` only). Vite's strict-ESM dev server does not
        // forward unknown property access to default, so we must explicitly
        // pick `mod.default` before calling `crypto_pwhash`, etc. See
