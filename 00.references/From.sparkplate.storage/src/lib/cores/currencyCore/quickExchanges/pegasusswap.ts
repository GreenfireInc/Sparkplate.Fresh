// PegasusSwap Quick Exchange API Integration
// Global instant exchange service

export interface PegasusSwapConfig {
  apiKey?: string;
  baseUrl?: string;
  sandbox?: boolean;
}

export class PegasusSwapExchangeClass {
  private config: PegasusSwapConfig;
  private baseUrl: string;

  constructor(config: PegasusSwapConfig = {}) {
    this.config = config;
    this.baseUrl = config.baseUrl || (config.sandbox ? 'https://api-sandbox.pegasusswap.com' : 'https://api.pegasusswap.com');
  }
  ...
  static readonly info = {
    ...
    country: 'Global',
    founded: 2020,
    website: 'https://pegasusswap.com/',
    apiDocs: 'https://pegasusswap.com/api/',
    status: 'Active',
  ...
  static readonly socialMedia = {
    twitter: 'https://twitter.com/pegasusswap',
    telegram: 'https://t.me/pegasusswap',
    reddit: 'https://www.reddit.com/r/pegasusswap/',
    youtube: 'https://www.youtube.com/pegasusswap',
    facebook: 'https://www.facebook.com/pegasusswap',
    linkedin: 'https://www.linkedin.com/company/pegasusswap',
  };

  /**
   * Make request to PegasusSwap API
   */
  private async makeRequest(endpoint: string, params: Record<string, any> = {}): Promise<any> {
    ...
    if (!response.ok) {
      throw new Error(`PegasusSwap API error: ${response.status} ${response.statusText}`);
    }

    const result = await response.json();
    if (result.error) {
      throw new Error(`PegasusSwap API error: ${result.error}`);
    }
...
export const PegasusSwapExchange = {
  // Quick access methods
  async getCurrencies(): Promise<string[]> {
    const instance = new PegasusSwapExchangeClass();
  ...
  // Static properties
  info: PegasusSwapExchangeClass.info,
  socialMedia: PegasusSwapExchangeClass.socialMedia,
};

// Export the class as default
export default PegasusSwapExchange;
