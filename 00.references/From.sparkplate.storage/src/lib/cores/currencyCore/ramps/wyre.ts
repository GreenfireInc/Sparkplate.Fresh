// Wyre Ramp API Integration
// Global crypto payment infrastructure
...
export class WyreRampClass {
  private config: WyreConfig;
  private baseUrl: string;

  constructor(config: WyreConfig = {}) {
    this.config = config;
    this.baseUrl = config.baseUrl || (config.sandbox ? 'https://api-sandbox.sendwyre.com' : 'https://api.sendwyre.com');
  }
  ...
  static readonly info = {
    ...
    apiDocs: 'https://docs.sendwyre.com/',
    status: 'Active',
    supportedCurrencies: '50+',
    features: ['Credit Card', 'Bank Transfer', 'Wire Transfer', 'Global Coverage'],
  };

  // Social Media Links
  static readonly socialMedia = {
    twitter: 'https://twitter.com/sendwyre',
    telegram: 'https://t.me/sendwyre',
    reddit: 'https://www.reddit.com/r/sendwyre/',
    youtube: 'https://www.youtube.com/sendwyre',
    facebook: 'https://www.facebook.com/sendwyre',
    linkedin: 'https://www.linkedin.com/company/sendwyre',
  };

  /**
   * Make request to Wyre API
   */
  private async makeRequest(endpoint: string, params: Record<string, any> = {}): Promise<any> {
    ...
    if (!response.ok) {
      throw new Error(`Wyre API error: ${response.status} ${response.statusText}`);
    }

    const result = await response.json();
    if (result.error) {
      throw new Error(`Wyre API error: ${result.error}`);
    }
...
export const WyreRamp = {
  // Quick access methods
  async getCurrencies(): Promise<any[]> {
    const instance = new WyreRampClass();
  ...
  // Static properties
  info: WyreRampClass.info,
  socialMedia: WyreRampClass.socialMedia,
};

// Export the class as default
export default WyreRamp;
