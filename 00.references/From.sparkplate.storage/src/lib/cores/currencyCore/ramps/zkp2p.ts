// ZKP2P Ramp API Integration
// Privacy-focused P2P platform
...
export class ZKP2PRampClass {
  private config: ZKP2PConfig;
  private baseUrl: string;

  constructor(config: ZKP2PConfig = {}) {
    this.config = config;
    this.baseUrl = config.baseUrl || (config.sandbox ? 'https://api-sandbox.zkp2p.io' : 'https://api.zkp2p.io');
  }
  ...
  static readonly info = {
    ...
    country: 'Global',
    founded: 2022,
    website: 'https://zkp2p.io/',
    apiDocs: 'https://zkp2p.io/api/',
    status: 'Active',
    supportedCurrencies: '20+',
    features: ['Privacy-focused', 'P2P Trading', 'Zero-Knowledge', 'Decentralized'],
  };

  // Social Media Links
  static readonly socialMedia = {
    twitter: 'https://twitter.com/zkp2p',
    telegram: 'https://t.me/zkp2p',
    reddit: 'https://www.reddit.com/r/zkp2p/',
    youtube: 'https://www.youtube.com/zkp2p',
    facebook: 'https://www.facebook.com/zkp2p',
    linkedin: 'https://www.linkedin.com/company/zkp2p',
  };

  /**
   * Make request to ZKP2P API
   */
  private async makeRequest(endpoint: string, params: Record<string, any> = {}): Promise<any> {
    ...
    if (!response.ok) {
      throw new Error(`ZKP2P API error: ${response.status} ${response.statusText}`);
    }

    const result = await response.json();
    if (result.error) {
      throw new Error(`ZKP2P API error: ${result.error}`);
    }
...
export const ZKP2PRamp = {
  // Quick access methods
  async getCurrencies(): Promise<any[]> {
    const instance = new ZKP2PRampClass();
  ...
  // Static properties
  info: ZKP2PRampClass.info,
  socialMedia: ZKP2PRampClass.socialMedia,
};

// Export the class as default
export default ZKP2PRamp;
