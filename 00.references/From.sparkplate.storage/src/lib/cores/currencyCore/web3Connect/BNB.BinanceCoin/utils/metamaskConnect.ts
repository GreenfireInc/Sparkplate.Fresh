 ...
 * 
 * Handles MetaMask browser extension wallet connection for BSC.
 * BSC is EVM-compatible, so MetaMask works natively.
 */
...
class MetaMaskConnect {
   ...
   */
  async switchToBSC(network: 'mainnet' | 'testnet' = 'mainnet') {
    if (!this.isAvailable()) {
      throw new Error('MetaMask extension not installed');
    }

    const networkConfig = bscNetworks[network];
   ...
   */
  async isOnBSC(): Promise<boolean> {
    const chainId = await this.getChainId();
    if (!chainId) return false;
    
    const chainIdNum = parseInt(chainId, 16);
    return chainIdNum === 56 || chainIdNum === 97;
  }
}
