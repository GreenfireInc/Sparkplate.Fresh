/**
 * Algorand WalletConnect Integration
 * 
 * Handles WalletConnect v2 protocol for Algorand dApp connections.
 * Similar to Ethereum WalletConnect but adapted for Algorand's transaction model.
 */

import { Core } from '@walletconnect/core';
import { Web3Wallet } from '@walletconnect/web3wallet';
import { buildApprovedNamespaces, getSdkError } from '@walletconnect/utils';
import { algoMethods } from './methods/algoRPCMethods';
...
class AlgorandWalletConnect {
  ...
  constructor() {
    this.requestTypes = {
      transactionRequests: ['algo_sendTransaction', 'algo_signTransaction'],
      signMessageRequests: ['algo_signMessage'],
      sessionRequest: 'session_request',
      sessionProposal: 'session_proposal',
      otherRequests: ['algo_sendRawTransaction', 'algo_getAccountInfo']
    };
  }
  ...
  async setHooks(modal: any, gtag: any, dispatch: any) {
    ...
    // Dynamically import DappCallRequest component
    // const DappCallRequest = await import('@/components/web3Connect/DappCallRequest.vue');
  ...
  async handleSessionProposal({ 
    ...
    // Build Algorand accounts
    const algoChains = [
      'algorand:416001', // Mainnet
      'algorand:416002'  // Testnet
    ];

    const accounts: string[] = [];
    algoChains.forEach((chain) => {
      wallets.forEach((wallet) => {
        const algoAccountString = `${chain}:${wallet.address}`;
        accounts.push(algoAccountString);
      });
    });
    ...
    const approvedNamespaces = buildApprovedNamespaces({
      ...
      supportedNamespaces: {
        algorand: {
          chains: algoChains,
          methods: Object.keys(algoMethods),
  ...
  async handleSessionRequest({ 
    ...
    if (!approved) {
      ...
      const network = chainId === '416001' ? 'mainnet' : 'testnet';
      
      const result = await algoMethods[method as keyof typeof algoMethods]({
        wallet,
        params,
        network,
  ...
  getAddressFromRequestParams(method: string, params: any[]): string | undefined {
    ...
    if (requestTypes.transactionRequests.includes(method)) {
      ...
      return params[1]?.address || params[0]?.address;
    }
  }
}

export default new AlgorandWalletConnect();
