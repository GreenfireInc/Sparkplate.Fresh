 ...
 * 
 * Handles WalletConnect v2 protocol for Tron dApp connections.
 * Tron uses its own namespace in WalletConnect.
 */

import { Core } from '@walletconnect/core';
import { Web3Wallet } from '@walletconnect/web3wallet';
import { buildApprovedNamespaces, getSdkError } from '@walletconnect/utils';
import { trxMethods } from './trxRPCMethods';

// App metadata for WalletConnect
const appMetadata = {
  name: 'LoginStandard',
  description: 'LoginStandard - Tron Wallet',
...
class TRXWalletConnect {
  ...
  constructor() {
    this.requestTypes = {
      transactionRequests: ['trx_signAndSendTransaction', 'trx_signTransaction'],
      signMessageRequests: ['trx_signMessage'],
      sessionRequest: 'session_request',
      sessionProposal: 'session_proposal',
      otherRequests: ['trx_sendRawTransaction', 'trx_getBalance', 'trx_getAccount']
    };
  }
  ...
  async handleSessionProposal({ 
    ...
    trxChains.forEach((chain) => {
      wallets.forEach((wallet) => {
        const trxAccountString = `${chain}:${wallet.address}`;
        accounts.push(trxAccountString);
      });
    });
    ...
    const approvedNamespaces = buildApprovedNamespaces({
      ...
      supportedNamespaces: {
        tron: {
          chains: trxChains,
          methods: Object.keys(trxMethods),
  ...
  async handleSessionRequest({ 
    ...
    if (!approved) {
      ...
      const result = await trxMethods[method as keyof typeof trxMethods]({
        wallet,
        params,
...
export default new TRXWalletConnect();
