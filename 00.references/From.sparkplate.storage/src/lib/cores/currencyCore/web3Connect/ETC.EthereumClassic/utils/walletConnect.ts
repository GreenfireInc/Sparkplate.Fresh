/**
 * Ethereum Classic WalletConnect Integration
 * 
 * Handles WalletConnect v2 protocol for ETC dApp connections.
 * ETC is EVM-compatible, so it uses EIP-155 namespace like Ethereum.
 */

import { Core } from '@walletconnect/core';
import { Web3Wallet } from '@walletconnect/web3wallet';
import { buildApprovedNamespaces, getSdkError } from '@walletconnect/utils';
import { etcMethods } from './etcRPCMethods';

// App metadata for WalletConnect
const appMetadata = {
  name: 'LoginStandard',
  description: 'LoginStandard - Ethereum Classic Wallet',
...
class ETCWalletConnect {
  ...
  constructor() {
    this.requestTypes = {
      transactionRequests: ['etc_sendTransaction', 'etc_signTransaction'],
      typedRequests: ['etc_signTypedData_v4'],
      signMessageRequests: ['etc_sign', 'etc_personal_sign'],
      sessionRequest: 'session_request',
      sessionProposal: 'session_proposal',
      otherRequests: ['etc_sendRawTransaction', 'etc_switchEthereumChain', 'etc_addEthereumChain']
    };
  }
  ...
  async handleSessionProposal({ 
    ...
    // Build EIP-155 accounts for ETC
    const etcChains = [
      'eip155:61',  // ETC Mainnet
      'eip155:63'   // Mordor Testnet
    ];

    const accounts: string[] = [];
    etcChains.forEach((chain) => {
    ...
    const approvedNamespaces = buildApprovedNamespaces({
      ...
      supportedNamespaces: {
        eip155: {
          chains: etcChains,
          methods: Object.keys(etcMethods),
  ...
  async handleSessionRequest({ 
    ...
    if (!approved) {
      ...
      const network = parseInt(chainId) === 61 ? 'mainnet' : 'testnet';
      
      // Map WalletConnect methods to ETC methods
      const methodMap: Record<string, string> = {
        'eth_sendTransaction': 'etc_sendTransaction',
        'eth_signTransaction': 'etc_signTransaction',
        'eth_signTypedData_v4': 'etc_signTypedData_v4',
        'eth_sign': 'etc_sign',
        'personal_sign': 'etc_personal_sign',
        'eth_sendRawTransaction': 'etc_sendRawTransaction',
        'wallet_switchEthereumChain': 'etc_switchEthereumChain',
        'wallet_addEthereumChain': 'etc_addEthereumChain'
      };

      const etcMethod = methodMap[method] || method;
      
      const result = await etcMethods[etcMethod as keyof typeof etcMethods]({
        wallet,
        params,
        network,
  ...
  getAddressFromRequestParams(method: string, params: any[]): string | undefined {
    ...
    if (requestTypes.transactionRequests.includes(method)) {
      ...
      return method === 'etc_personal_sign' ? params[1] : params[0];
    }
  }
}

export default new ETCWalletConnect();
