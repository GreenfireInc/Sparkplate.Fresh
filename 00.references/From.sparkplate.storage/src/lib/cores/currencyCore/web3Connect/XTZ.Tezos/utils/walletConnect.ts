/**
 * Tezos WalletConnect Integration
 * 
 * Handles WalletConnect v2 protocol for Tezos dApp connections.
 * Tezos uses its own namespace in WalletConnect.
 */

import { Core } from '@walletconnect/core';
import { Web3Wallet } from '@walletconnect/web3wallet';
import { buildApprovedNamespaces, getSdkError } from '@walletconnect/utils';
import { xtzMethods } from './xtzRPCMethods';

// App metadata for WalletConnect
const appMetadata = {
  name: 'LoginStandard',
  description: 'LoginStandard - Tezos Wallet',
...
class XTZWalletConnect {
  ...
  constructor() {
    this.requestTypes = {
      transactionRequests: ['xtz_signAndSendTransaction', 'xtz_signTransaction'],
      signMessageRequests: ['xtz_signMessage'],
      sessionRequest: 'session_request',
      sessionProposal: 'session_proposal',
      otherRequests: ['xtz_sendTransaction', 'xtz_getBalance', 'xtz_getAccountInfo']
    };
  }
  ...
  async handleSessionProposal({ 
    ...
    tezosChains.forEach((chain) => {
      wallets.forEach((wallet) => {
        const tezosAccountString = `${chain}:${wallet.address}`;
        accounts.push(tezosAccountString);
      });
    });
    ...
    const approvedNamespaces = buildApprovedNamespaces({
      ...
      supportedNamespaces: {
        tezos: {
          chains: tezosChains,
          methods: Object.keys(xtzMethods),
  ...
  async handleSessionRequest({ 
    ...
    if (!approved) {
      ...
      const result = await xtzMethods[method as keyof typeof xtzMethods]({
        wallet,
        params,
  ...
  getAddressFromRequestParams(method: string, params: any[]): string | undefined {
    ...
    if (requestTypes.transactionRequests.includes(method)) {
      return params[0]?.from || params[0]?.to;
...
export default new XTZWalletConnect();
