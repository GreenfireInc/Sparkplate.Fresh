/**
 * Ethereum (ETH) Smart Contract Escrow System
 * Solidity escrow with EIP-1559 support
 */
...
export class EthereumEscrowContractClient {
  ...
  async depositToEscrow(playerPrivateKey: string, betAmountETH: string): Promise<string> {
  ...
  async payoutWinner(serverPrivateKey: string, winnerAddress: string): Promise<string> {
    ...
    const messageHash = ethers.solidityPackedKeccak256(['address', 'address'], [winnerAddress, this.contractAddress]);
...
export default EthereumEscrowContractClient;
