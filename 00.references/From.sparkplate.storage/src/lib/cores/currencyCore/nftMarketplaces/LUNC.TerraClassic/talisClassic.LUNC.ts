// Talis Classic Marketplace Information
// NFT marketplace on Terra Classic blockchain
// Source: Research compiled from multiple sources

export interface TalisClassicListing {
  tokenId: string;
...
export interface TalisClassicCollection {
  collectionAddress: string;
  name: string;
  description?: string;
  creator: string;
...
export interface TalisClassicPricing {
  tokenId: string;
  collectionAddress: string;
  currentPrice?: number;
  floorPrice?: number;
...
export const talisClassicMarketplace = {
  name: "Talis (Terra Classic)",
  blockchain: "Terra Classic",
  type: "NFT Marketplace",
  description: "NFT marketplace on Terra Classic blockchain. Features CW721 NFT trading, collections, and creator tools for the legacy Terra Classic ecosystem.",
  ...
  api: {
    endpoints: {
      mainnet: "https://api.talis.art/v1/classic/",
      classic: "https://api.talis.art/v1/classic/",
      base: "https://api.talis.art/v1/classic/",
    },
    pricing: {
...
async function getNFTPricing(collectionAddress: string, tokenId: string) {
  const response = await fetch(\`https://api.talis.art/v1/classic/nft/\${collectionAddress}/\${tokenId}/price\`);
  const pricing: TalisClassicPricing = await response.json();
  return pricing;
}

// Get collection floor price
async function getCollectionFloorPrice(collectionAddress: string) {
  const response = await fetch(\`https://api.talis.art/v1/classic/collections/\${collectionAddress}/floor-price\`);
...
async function searchNFTs(keyword: string) {
  const response = await fetch(\`https://api.talis.art/v1/classic/nfts/search?keyword=\${encodeURIComponent(keyword)}\`);
  const data = await response.json();
  return data.results;
}
    `,
...
export default talisClassicMarketplace;
