/**
 * Mintscan API for Terra Classic (LUNC)
 * 
...
// API Configuration
export interface MintscanClassicConfig {
  baseURL?: string;
  timeout?: number;
  retries?: number;
}
...
// Account Information
export interface MintscanClassicAccountInfo {
  account: {
    address: string;
    coins: Array<{
      denom: string;
...
// Balance Information
export interface MintscanClassicBalance {
  denom: string;
  amount: string;
  available: string;
  delegated: string;
...
// Transaction
export interface MintscanClassicTransaction {
  txhash: string;
  height: string;
  code?: number;
  timestamp: string;
...
// Delegation
export interface MintscanClassicDelegation {
  delegation: {
...
// Validator
export interface MintscanClassicValidator {
...
// Reward
export interface MintscanClassicReward {
  validator_address: string;
  reward: Array<{
    denom: string;
    amount: string;
...
export class MintscanClassicAPI {
  private client: AxiosInstance;
  private config: Required<MintscanClassicConfig>;

  constructor(config: MintscanClassicConfig = {}) {
    this.config = {
      baseURL: config.baseURL || 'https://api-terra-classic.cosmostation.io',
   ...
   */
  async getAccountInfo(address: string): Promise<MintscanClassicAccountInfo> {
   ...
   * Get account balance
   */
  async getBalance(address: string): Promise<MintscanClassicBalance[]> {
   ...
   */
  async getTransaction(txHash: string): Promise<MintscanClassicTransaction> {
  ...
  async getTransactionHistory(
    ...
    limit: number = 50,
    offset: number = 0
  ): Promise<MintscanClassicTransaction[]> {
   ...
   */
  async getDelegations(address: string): Promise<MintscanClassicDelegation[]> {
   ...
   */
  async getRewards(address: string): Promise<MintscanClassicReward[]> {
   ...
   * Get all validators
   */
  async getValidators(status: string = 'BOND_STATUS_BONDED'): Promise<MintscanClassicValidator[]> {
   ...
   */
  async getValidator(validatorAddress: string): Promise<MintscanClassicValidator> {
...
// Singleton instance
export const mintscanClassicAPI = new MintscanClassicAPI();
