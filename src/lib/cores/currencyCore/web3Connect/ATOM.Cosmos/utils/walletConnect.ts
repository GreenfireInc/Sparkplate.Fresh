/**
 * Cosmos WalletConnect Integration
 * 
 * Handles WalletConnect v2 protocol for Cosmos dApp connections.
 * Supports Cosmos Hub and other Cosmos ecosystem chains.
 */

import { Core } from '@walletconnect/core';
import { Web3Wallet } from '@walletconnect/web3wallet';
import { buildApprovedNamespaces, getSdkError } from '@walletconnect/utils';
import { cosmosMethods } from './cosmosRPCMethods';

// App metadata for WalletConnect
const appMetadata = {
  name: 'LoginStandard',
  description: 'LoginStandard - Cosmos Wallet',
...
class CosmosWalletConnect {
  ...
  constructor() {
    this.requestTypes = {
      transactionRequests: ['cosmos_sendTransaction', 'cosmos_sendTokens', 'cosmos_signTransaction'],
      sessionRequest: 'session_request',
      sessionProposal: 'session_proposal',
      otherRequests: ['cosmos_getAccount', 'cosmos_getBalance', 'cosmos_getTransaction']
    };
  }
  ...
  async handleSessionProposal({ 
    ...
    // Build Cosmos accounts for multiple chains
    const cosmosChains = [
      'cosmos:cosmoshub-4',      // Cosmos Hub Mainnet
      'cosmos:osmosis-1',        // Osmosis
      'cosmos:juno-1',           // Juno
      'cosmos:theta-testnet-001' // Cosmos Hub Testnet
    ];
    ...
    const approvedNamespaces = buildApprovedNamespaces({
      ...
      supportedNamespaces: {
        cosmos: {
          chains: cosmosChains,
          methods: Object.keys(cosmosMethods),
  ...
  async handleSessionRequest({ 
    ...
    if (!approved) {
      ...
      if (chainId.includes('testnet')) {
        ...
        network = 'osmosis';
      } else if (chainId === 'juno-1') {
        network = 'juno';
      }
      
      const result = await cosmosMethods[method as keyof typeof cosmosMethods]({
        wallet,
        params,
        network,
        chainId,
...
export default new CosmosWalletConnect();
