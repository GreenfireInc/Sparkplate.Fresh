/**
 * Stacks WalletConnect Integration
 * 
 * Handles WalletConnect v2 protocol for Stacks dApp connections.
 * Stacks uses its own namespace in WalletConnect.
 */

import { Core } from '@walletconnect/core';
import { Web3Wallet } from '@walletconnect/web3wallet';
import { buildApprovedNamespaces, getSdkError } from '@walletconnect/utils';
import { stxMethods } from './stxRPCMethods';

// App metadata for WalletConnect
const appMetadata = {
  name: 'LoginStandard',
  description: 'LoginStandard - Stacks Wallet',
...
class STXWalletConnect {
  ...
  constructor() {
    this.requestTypes = {
      transactionRequests: ['stx_signAndSendTransaction', 'stx_signTransaction'],
      signMessageRequests: ['stx_signMessage'],
      sessionRequest: 'session_request',
      sessionProposal: 'session_proposal',
      otherRequests: ['stx_sendTransaction', 'stx_getBalance', 'stx_getAccountInfo']
    };
  }
  ...
  async handleSessionProposal({ 
    ...
    stxChains.forEach((chain) => {
      wallets.forEach((wallet) => {
        const stxAccountString = `${chain}:${wallet.address}`;
        accounts.push(stxAccountString);
      });
    });
    ...
    const approvedNamespaces = buildApprovedNamespaces({
      ...
      supportedNamespaces: {
        stacks: {
          chains: stxChains,
          methods: Object.keys(stxMethods),
  ...
  async handleSessionRequest({ 
    ...
    if (!approved) {
      ...
      const result = await stxMethods[method as keyof typeof stxMethods]({
        wallet,
        params,
...
export default new STXWalletConnect();
