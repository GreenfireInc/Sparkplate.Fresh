/**
 * Waves WalletConnect Integration
 * 
 * Handles WalletConnect v2 protocol for Waves dApp connections.
 * Waves uses its own namespace in WalletConnect.
 */

import { Core } from '@walletconnect/core';
import { Web3Wallet } from '@walletconnect/web3wallet';
import { buildApprovedNamespaces, getSdkError } from '@walletconnect/utils';
import { wavesMethods } from './wavesRPCMethods';

// App metadata for WalletConnect
const appMetadata = {
  name: 'LoginStandard',
  description: 'LoginStandard - Waves Wallet',
...
class WavesWalletConnect {
  ...
  constructor() {
    this.requestTypes = {
      transactionRequests: ['waves_signAndSendTransaction', 'waves_signTransaction'],
      signMessageRequests: ['waves_signMessage'],
      sessionRequest: 'session_request',
      sessionProposal: 'session_proposal',
      otherRequests: ['waves_sendTransaction', 'waves_getBalance', 'waves_getAccountInfo']
    };
  }
  ...
  async handleSessionProposal({ 
    ...
    wavesChains.forEach((chain) => {
      wallets.forEach((wallet) => {
        const wavesAccountString = `${chain}:${wallet.address}`;
        accounts.push(wavesAccountString);
      });
    });
    ...
    const approvedNamespaces = buildApprovedNamespaces({
      ...
      supportedNamespaces: {
        waves: {
          chains: wavesChains,
          methods: Object.keys(wavesMethods),
  ...
  async handleSessionRequest({ 
    ...
    if (!approved) {
      ...
      const result = await wavesMethods[method as keyof typeof wavesMethods]({
        wallet,
        params,
  ...
  getAddressFromRequestParams(method: string, params: any[]): string | undefined {
    ...
    if (requestTypes.transactionRequests.includes(method)) {
      return params[0]?.from || params[0]?.sender;
...
export default new WavesWalletConnect();
