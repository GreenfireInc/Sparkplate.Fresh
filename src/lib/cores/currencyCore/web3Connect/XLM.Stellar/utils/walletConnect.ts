/**
 * Stellar WalletConnect Integration
 * 
 * Handles WalletConnect v2 protocol for Stellar dApp connections.
 * Stellar uses its own namespace in WalletConnect.
 */

import { Core } from '@walletconnect/core';
import { Web3Wallet } from '@walletconnect/web3wallet';
import { buildApprovedNamespaces, getSdkError } from '@walletconnect/utils';
import { xlmMethods } from './xlmRPCMethods';

// App metadata for WalletConnect
const appMetadata = {
  name: 'LoginStandard',
  description: 'LoginStandard - Stellar Wallet',
...
class XLMWalletConnect {
  ...
  constructor() {
    this.requestTypes = {
      transactionRequests: ['xlm_signAndSendTransaction', 'xlm_signTransaction'],
      signMessageRequests: ['xlm_signMessage'],
      sessionRequest: 'session_request',
      sessionProposal: 'session_proposal',
      otherRequests: ['xlm_sendTransaction', 'xlm_getBalance', 'xlm_getAccountInfo']
    };
  }
  ...
  async handleSessionProposal({ 
    ...
    xlmChains.forEach((chain) => {
      wallets.forEach((wallet) => {
        const xlmAccountString = `${chain}:${wallet.address}`;
        accounts.push(xlmAccountString);
      });
    });
    ...
    const approvedNamespaces = buildApprovedNamespaces({
      ...
      supportedNamespaces: {
        stellar: {
          chains: xlmChains,
          methods: Object.keys(xlmMethods),
  ...
  async handleSessionRequest({ 
    ...
    if (!approved) {
      ...
      const result = await xlmMethods[method as keyof typeof xlmMethods]({
        wallet,
        params,
  ...
  getAddressFromRequestParams(method: string, params: any[]): string | undefined {
    ...
    if (requestTypes.transactionRequests.includes(method)) {
      return params[0]?.from || params[0]?.source;
...
export default new XLMWalletConnect();
