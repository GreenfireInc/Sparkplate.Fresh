// Swapuz Quick Exchange API Integration
// Global instant exchange service

export interface SwapuzConfig {
  apiKey?: string;
  baseUrl?: string;
  sandbox?: boolean;
}

export class SwapuzExchangeClass {
  private config: SwapuzConfig;
  private baseUrl: string;

  constructor(config: SwapuzConfig = {}) {
    this.config = config;
    this.baseUrl = config.baseUrl || (config.sandbox ? 'https://api-sandbox.swapuz.com' : 'https://api.swapuz.com');
  }
  ...
  static readonly socialMedia = {
    twitter: 'https://twitter.com/swapuz',
    telegram: 'https://t.me/swapuz',
    reddit: 'https://www.reddit.com/r/swapuz/',
    youtube: 'https://www.youtube.com/swapuz',
    facebook: 'https://www.facebook.com/swapuz',
    linkedin: 'https://www.linkedin.com/company/swapuz',
  };

  /**
   * Make request to Swapuz API
   */
  private async makeRequest(endpoint: string, params: Record<string, any> = {}): Promise<any> {
    ...
    if (!response.ok) {
      throw new Error(`Swapuz API error: ${response.status} ${response.statusText}`);
    }

    const result = await response.json();
    if (result.error) {
      throw new Error(`Swapuz API error: ${result.error}`);
    }
...
export const SwapuzExchange = {
  // Quick access methods
  async getCurrencies(): Promise<string[]> {
    const instance = new SwapuzExchangeClass();
  ...
  // Static properties
  info: SwapuzExchangeClass.info,
  socialMedia: SwapuzExchangeClass.socialMedia,
};

// Export the class as default
export default SwapuzExchange;
