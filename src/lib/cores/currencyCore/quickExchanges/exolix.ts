// Exolix Quick Exchange API Integration
// Global instant exchange service

export interface ExolixConfig {
  apiKey?: string;
  baseUrl?: string;
  sandbox?: boolean;
}

export class ExolixExchangeClass {
  private config: ExolixConfig;
  private baseUrl: string;

  constructor(config: ExolixConfig = {}) {
    this.config = config;
    this.baseUrl = config.baseUrl || (config.sandbox ? 'https://api-sandbox.exolix.com' : 'https://api.exolix.com');
  }
  ...
  static readonly socialMedia = {
    twitter: 'https://twitter.com/exolix',
    telegram: 'https://t.me/exolix',
    reddit: 'https://www.reddit.com/r/exolix/',
    youtube: 'https://www.youtube.com/exolix',
    facebook: 'https://www.facebook.com/exolix',
    linkedin: 'https://www.linkedin.com/company/exolix',
  };

  /**
   * Make request to Exolix API
   */
  private async makeRequest(endpoint: string, params: Record<string, any> = {}): Promise<any> {
    ...
    if (!response.ok) {
      throw new Error(`Exolix API error: ${response.status} ${response.statusText}`);
    }

    const result = await response.json();
    if (result.error) {
      throw new Error(`Exolix API error: ${result.error}`);
    }
...
export const ExolixExchange = {
  // Quick access methods
  async getCurrencies(): Promise<string[]> {
    const instance = new ExolixExchangeClass();
  ...
  // Static properties
  info: ExolixExchangeClass.info,
  socialMedia: ExolixExchangeClass.socialMedia,
};

// Export the class as default
export default ExolixExchange;
