// EasyRabbit Quick Exchange API Integration
// Global instant exchange service

export interface EasyRabbitConfig {
  apiKey?: string;
  baseUrl?: string;
  sandbox?: boolean;
}

export class EasyRabbitExchangeClass {
  private config: EasyRabbitConfig;
  private baseUrl: string;

  constructor(config: EasyRabbitConfig = {}) {
    this.config = config;
    this.baseUrl = config.baseUrl || (config.sandbox ? 'https://api-sandbox.easyrabbit.io' : 'https://api.easyrabbit.io');
  }
  ...
  static readonly info = {
    ...
    country: 'Global',
    founded: 2020,
    website: 'https://easyrabbit.io/',
    apiDocs: 'https://easyrabbit.io/api/',
    status: 'Active',
  ...
  static readonly socialMedia = {
    twitter: 'https://twitter.com/easyrabbit',
    telegram: 'https://t.me/easyrabbit',
    reddit: 'https://www.reddit.com/r/easyrabbit/',
    youtube: 'https://www.youtube.com/easyrabbit',
    facebook: 'https://www.facebook.com/easyrabbit',
    linkedin: 'https://www.linkedin.com/company/easyrabbit',
  };

  /**
   * Make request to EasyRabbit API
   */
  private async makeRequest(endpoint: string, params: Record<string, any> = {}): Promise<any> {
    ...
    if (!response.ok) {
      throw new Error(`EasyRabbit API error: ${response.status} ${response.statusText}`);
    }

    const result = await response.json();
    if (result.error) {
      throw new Error(`EasyRabbit API error: ${result.error}`);
    }
...
export const EasyRabbitExchange = {
  // Quick access methods
  async getCurrencies(): Promise<string[]> {
    const instance = new EasyRabbitExchangeClass();
  ...
  // Static properties
  info: EasyRabbitExchangeClass.info,
  socialMedia: EasyRabbitExchangeClass.socialMedia,
};

// Export the class as default
export default EasyRabbitExchange;
