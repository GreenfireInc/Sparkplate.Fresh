// Nanswap Quick Exchange API Integration
// Global instant exchange service

export interface NanswapConfig {
  apiKey?: string;
  baseUrl?: string;
  sandbox?: boolean;
}

export class NanswapExchangeClass {
  private config: NanswapConfig;
  private baseUrl: string;

  constructor(config: NanswapConfig = {}) {
    this.config = config;
    this.baseUrl = config.baseUrl || (config.sandbox ? 'https://api-sandbox.nanswap.com' : 'https://api.nanswap.com');
  }
  ...
  static readonly socialMedia = {
    twitter: 'https://twitter.com/nanswap',
    telegram: 'https://t.me/nanswap',
    reddit: 'https://www.reddit.com/r/nanswap/',
    youtube: 'https://www.youtube.com/nanswap',
    facebook: 'https://www.facebook.com/nanswap',
    linkedin: 'https://www.linkedin.com/company/nanswap',
  };

  /**
   * Make request to Nanswap API
   */
  private async makeRequest(endpoint: string, params: Record<string, any> = {}): Promise<any> {
    ...
    if (!response.ok) {
      throw new Error(`Nanswap API error: ${response.status} ${response.statusText}`);
    }

    const result = await response.json();
    if (result.error) {
      throw new Error(`Nanswap API error: ${result.error}`);
    }
...
export const NanswapExchange = {
  // Quick access methods
  async getCurrencies(): Promise<string[]> {
    const instance = new NanswapExchangeClass();
  ...
  // Static properties
  info: NanswapExchangeClass.info,
  socialMedia: NanswapExchangeClass.socialMedia,
};

// Export the class as default
export default NanswapExchange;
