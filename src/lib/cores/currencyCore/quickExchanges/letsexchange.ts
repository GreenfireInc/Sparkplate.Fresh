// LetsExchange Quick Exchange API Integration
// Global instant exchange service

export interface LetsExchangeConfig {
  apiKey?: string;
  baseUrl?: string;
  sandbox?: boolean;
}

export class LetsExchangeExchangeClass {
  private config: LetsExchangeConfig;
  private baseUrl: string;

  constructor(config: LetsExchangeConfig = {}) {
    this.config = config;
    this.baseUrl = config.baseUrl || (config.sandbox ? 'https://api-sandbox.letsexchange.io' : 'https://api.letsexchange.io');
  }
  ...
  static readonly info = {
    ...
    apiDocs: 'https://letsexchange.io/api/',
    status: 'Active',
  ...
  static readonly socialMedia = {
    twitter: 'https://twitter.com/letsexchange',
    telegram: 'https://t.me/letsexchange',
    reddit: 'https://www.reddit.com/r/letsexchange/',
    youtube: 'https://www.youtube.com/letsexchange',
    facebook: 'https://www.facebook.com/letsexchange',
    linkedin: 'https://www.linkedin.com/company/letsexchange',
  };

  /**
   * Make request to LetsExchange API
   */
  private async makeRequest(endpoint: string, params: Record<string, any> = {}): Promise<any> {
    ...
    if (!response.ok) {
      throw new Error(`LetsExchange API error: ${response.status} ${response.statusText}`);
    }

    const result = await response.json();
    if (result.error) {
      throw new Error(`LetsExchange API error: ${result.error}`);
    }
...
export const LetsExchangeExchange = {
  // Quick access methods
  async getCurrencies(): Promise<string[]> {
    const instance = new LetsExchangeExchangeClass();
  ...
  // Static properties
  info: LetsExchangeExchangeClass.info,
  socialMedia: LetsExchangeExchangeClass.socialMedia,
};

// Export the class as default
export default LetsExchangeExchange;
