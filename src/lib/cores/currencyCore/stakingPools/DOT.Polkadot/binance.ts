// Binance Staking Pool for Polkadot (DOT)
...
export const BinanceStaking = {
  ...
  description: "Flexible and locked DOT staking on Binance with competitive rates",
  minimumStake: "0.1 DOT",
  apy: "Up to 12%",
...
client.stakingProductList({ product: 'STAKING' })
  .then(products => {
    const dotProducts = products.filter(p => p.asset === 'DOT');
    console.log('DOT staking products:', dotProducts);
  })
...
const stakeData = {
  product: 'STAKING',
  productId: 'DOT*30', // 30-day locked staking
...
client.stakingRecord({ product: 'STAKING' })
  .then(records => {
    const dotRecords = records.filter(r => r.asset === 'DOT');
    console.log('DOT staking history:', dotRecords);
  })
  .catch(err => console.error('Error:', err));
    `,
  },
...
export async function getBinanceStakingProducts() {
  try {
    ...
    if (response.ok) {
      ...
      return data.filter(product => product.asset === 'DOT');
    }
  } catch (error) {
...
// Helper function to get DOT price from Binance
