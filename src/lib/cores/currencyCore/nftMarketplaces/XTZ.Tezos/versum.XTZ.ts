// Versum Marketplace Information
// NFT marketplace on Tezos blockchain
// Source: Research compiled from multiple sources

export interface VersumListing {
  tokenId: string;
...
export interface VersumCollection {
  contractAddress: string;
  name: string;
  description?: string;
  creator: string;
...
export interface VersumPricing {
  tokenId: string;
  contractAddress: string;
  currentPrice?: number;
  floorPrice?: number;
...
export const versumMarketplace = {
  name: "Versum",
  blockchain: "Tezos",
  type: "NFT Marketplace",
  description: "NFT marketplace on Tezos blockchain. Features FA2 (TZIP-12) NFT trading, collections, and marketplace tools for the Tezos ecosystem.",
  
  urls: {
    main: "https://versum.xyz/",
    app: "https://versum.xyz/",
    docs: "https://docs.versum.xyz/",
  },
  
  api: {
    endpoints: {
      mainnet: "https://api.versum.xyz/v1/",
      tezos: "https://api.versum.xyz/v1/",
      base: "https://api.versum.xyz/v1/",
    },
    pricing: {
    ...
    documentation: "https://docs.versum.xyz/api",
    rateLimit: "Rate limits apply (check documentation for current limits)",
    requiresApiKey: false,
    authentication: {
      type: "Optional API Key for higher limits",
      headerName: "X-API-Key",
      getApiKey: "https://versum.xyz/api",
    },
  },
  ...
  sdk: {
    ...
    documentation: "https://docs.versum.xyz/developers",
...
async function getNFTPricing(contractAddress: string, tokenId: string) {
  const response = await fetch(\`https://api.versum.xyz/v1/tokens/\${contractAddress}/\${tokenId}/price\`);
  const pricing: VersumPricing = await response.json();
  return pricing;
}

// Get collection floor price
async function getCollectionFloorPrice(contractAddress: string) {
  const response = await fetch(\`https://api.versum.xyz/v1/collections/\${contractAddress}/floor-price\`);
...
async function searchNFTs(keyword: string) {
  const response = await fetch(\`https://api.versum.xyz/v1/tokens/search?keyword=\${encodeURIComponent(keyword)}\`);
  const data = await response.json();
  return data.results;
}
    `,
  ...
  socialMedia: {
    twitter: "https://twitter.com/versum_xyz",
    discord: "https://discord.gg/versum",
    telegram: null,
    medium: null,
    github: "https://github.com/versum",
  },
...
export default versumMarketplace;
