// Terra (LUNA) NFT Minting with Arweave Storage
// Mints CW721 NFTs on Terra with assets stored permanently on Arweave

/**
 * OVERVIEW:
 * 
 * This module mints NFTs on Terra (LUNA 2.0) with assets stored permanently on Arweave.
 * - Upload files directly to Arweave (permanent storage)
 * - Upload metadata to Arweave (immutable)
 * - Create CW721 NFT on Terra (via CosmWasm)
 * - Link NFT metadata to Arweave URLs
 * - Best of both worlds: Terra fast transactions + Arweave permanent storage
 * 
 * BENEFITS:
 * - Terra: Fast, low-cost, IBC compatible, Cosmos ecosystem
 ...
 * - Terra: ~0.001-0.01 LUNA per mint (gas fee)
 * - Total: ~0.01-0.02 AR + 0.001-0.01 LUNA per NFT
 * 
 * TERRA NFT STANDARD:
...
export interface TerraArweaveNFTConfig {
  rpcEndpoint?: string; // Terra RPC endpoint
  chainId?: string; // Terra chain ID
...
export interface MintResult {
  ...
  transactionHash: string; // Terra transaction hash
...
export class TerraArweaveNFTMinter {
  private rpcEndpoint: string;
  private chainId: string;
  private arweave: Arweave;

  constructor(config: TerraArweaveNFTConfig = {}) {
    this.rpcEndpoint = config.rpcEndpoint || 'https://phoenix-lcd.terra.dev';
    this.chainId = config.chainId || 'phoenix-1'; // Terra 2.0 mainnet
  ...
  private async uploadFileToArweave(
    ...
    try {
      ...
      transaction.addTag('App-Name', 'Terra-NFT');
  ...
  /**
   * Mint NFT on Terra with assets stored on Arweave
  ...
  async mintNFT(
    ...
    console.log('Starting Terra NFT minting with Arweave storage...');
    ...
    // 5. Connect to Terra and mint NFT
    ...
    console.log(`NFT minted on Terra: Token ID ${tokenId}`);
    ...
    const feeLUNA = (parseInt(feeAmount) / 1000000).toFixed(6); // uluna to LUNA
    ...
    const viewBlockUrl = network === 'mainnet'
      ? `https://viewblock.io/terra2/tx/${result.transactionHash}`
      : `https://testnet.viewblock.io/terra2/tx/${result.transactionHash}`;
  ...
  async mintMediaNFT(
    ...
    console.log('Starting Terra media NFT minting with Arweave storage...');
    ...
    const gasUsed = tx?.gasUsed?.toString() || '0';
    const feeAmount = tx?.tx.authInfo.fee?.amount?.[0]?.amount || '0';
    const feeLUNA = (parseInt(feeAmount) / 1000000).toFixed(6);
  ...
  /**
   * Check Terra account balance
   */
  async checkTerraBalance(address: string): Promise<{
    address: string;
    balanceLUNA: string;
    balanceULuna: string;
  }> {
    try {
      ...
      const balanceLUNA = (parseInt(balance.amount) / 1000000).toFixed(6);
      ...
      throw new Error(`Failed to check Terra balance: ${error.message}`);
    }
  }
 ...
 * USAGE EXAMPLES:
 * 
 * // Basic NFT minting on Terra with Arweave storage
 * import { TerraArweaveNFTMinter } from './LUNA.Terra.nft.arweaveDirect';
 * import { Secp256k1HdWallet } from '@cosmjs/amino';
 * import * as fs from 'fs';
 * 
 * // Initialize minter
 * const minter = new TerraArweaveNFTMinter({
 *   rpcEndpoint: 'https://phoenix-lcd.terra.dev',
 ...
 * 
 * // Check balances
 * const terraBalance = await minter.checkTerraBalance(account.address);
 * console.log(`Terra balance: ${terraBalance.balanceLUNA} LUNA`);
 * 
 ...
 *   {
 *     name: "Terra Art #1",
 *     description: "NFT on Terra 2.0, stored permanently on Arweave",
 *     attributes: [
 *       { trait_type: "Rarity", value: "Legendary" },
 *       { trait_type: "Blockchain", value: "Terra" },
 ...
 * console.log('Terra TX:', result.transactionHash);
 ...
 * console.log('Total Terra cost:', result.totalCost.terra.totalLUNA, 'LUNA');
 * 
 ...
 *   {
 *     name: "Animated Art #1",
 *     description: "Terra NFT with permanent Arweave storage",
 *     attributes: [
 ...
 * 
 * // Terra ecosystem revival: Post-collapse NFT collection
 * const revivalNFT = await minter.mintNFT(
 *   wallet,
 *   arweaveKey,
 *   cw721Contract,
 *   imageData,
 *   'revival.png',
 *   {
 *     name: "Terra Revival #1",
 *     description: "NFT commemorating Terra 2.0 ecosystem revival",
 *     attributes: [
 *       { trait_type: "Era", value: "Post-Revival" },
 *       { trait_type: "Significance", value: "Historical" }
 *     ]
 *   },
 *   'revival-1',
 *   account.address,
 ...
 * );
 * 
 * console.log('Terra revival NFT minted with permanent Arweave storage!');
 */

export default TerraArweaveNFTMinter;
