// Kriptomat Ramp API Integration
// European crypto platform

export interface KriptomatConfig {
  apiKey?: string;
  baseUrl?: string;
  sandbox?: boolean;
}

export class KriptomatRampClass {
  private config: KriptomatConfig;
  private baseUrl: string;

  constructor(config: KriptomatConfig = {}) {
    this.config = config;
    this.baseUrl = config.baseUrl || (config.sandbox ? 'https://api-sandbox.kriptomat.io' : 'https://api.kriptomat.io');
  }
  ...
  static readonly info = {
    ...
    apiDocs: 'https://kriptomat.io/api/',
    status: 'Active',
    supportedCurrencies: '30+',
    features: ['Licensed Exchange', 'Credit Card', 'Bank Transfer', 'European Focus'],
  };

  // Social Media Links
  static readonly socialMedia = {
    twitter: 'https://twitter.com/kriptomat',
    telegram: 'https://t.me/kriptomat',
    reddit: 'https://www.reddit.com/r/kriptomat/',
    youtube: 'https://www.youtube.com/kriptomat',
    facebook: 'https://www.facebook.com/kriptomat',
    linkedin: 'https://www.linkedin.com/company/kriptomat',
  };

  /**
   * Make request to Kriptomat API
   */
  private async makeRequest(endpoint: string, params: Record<string, any> = {}): Promise<any> {
    ...
    if (!response.ok) {
      throw new Error(`Kriptomat API error: ${response.status} ${response.statusText}`);
    }

    const result = await response.json();
    if (result.error) {
      throw new Error(`Kriptomat API error: ${result.error}`);
    }
...
export const KriptomatRamp = {
  async getCurrencies(): Promise<any[]> {
    const instance = new KriptomatRampClass();
  ...
  info: KriptomatRampClass.info,
  socialMedia: KriptomatRampClass.socialMedia,
};

export default KriptomatRamp;
