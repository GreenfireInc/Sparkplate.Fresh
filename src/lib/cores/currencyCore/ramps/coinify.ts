// Coinify Ramp API Integration
// European cryptocurrency broker

export interface CoinifyConfig {
  apiKey?: string;
  baseUrl?: string;
  sandbox?: boolean;
}

export class CoinifyRampClass {
  private config: CoinifyConfig;
  private baseUrl: string;

  constructor(config: CoinifyConfig = {}) {
    this.config = config;
    this.baseUrl = config.baseUrl || (config.sandbox ? 'https://api-sandbox.coinify.com' : 'https://api.coinify.com');
  }
  ...
  static readonly socialMedia = {
    twitter: 'https://twitter.com/coinify',
    telegram: 'https://t.me/coinify',
    reddit: 'https://www.reddit.com/r/coinify/',
    youtube: 'https://www.youtube.com/coinify',
    facebook: 'https://www.facebook.com/coinify',
    linkedin: 'https://www.linkedin.com/company/coinify',
  };

  /**
   * Make request to Coinify API
   */
  private async makeRequest(endpoint: string, params: Record<string, any> = {}): Promise<any> {
    ...
    if (!response.ok) {
      throw new Error(`Coinify API error: ${response.status} ${response.statusText}`);
    }

    const result = await response.json();
    if (result.error) {
      throw new Error(`Coinify API error: ${result.error}`);
    }
...
export const CoinifyRamp = {
  // Quick access methods
  async getCurrencies(): Promise<any[]> {
    const instance = new CoinifyRampClass();
  ...
  // Static properties
  info: CoinifyRampClass.info,
  socialMedia: CoinifyRampClass.socialMedia,
};

// Export the class as default
export default CoinifyRamp;
