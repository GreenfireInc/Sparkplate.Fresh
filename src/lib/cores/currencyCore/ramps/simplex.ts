...
export class SimplexRampClass {
  private config: SimplexConfig;
  private baseUrl: string;

  constructor(config: SimplexConfig = {}) {
    this.config = config;
    this.baseUrl = config.baseUrl || (config.sandbox ? 'https://api-sandbox.simplex.com' : 'https://api.simplex.com');
  }
  ...
  static readonly info = {
    ...
    apiDocs: 'https://www.simplex.com/api/',
    status: 'Active',
    supportedCurrencies: '50+',
    features: ['Credit Card', 'Bank Transfer', 'Licensed', 'Compliance'],
  };

  // Social Media Links
  static readonly socialMedia = {
    twitter: 'https://twitter.com/simplex',
    telegram: 'https://t.me/simplex',
    reddit: 'https://www.reddit.com/r/simplex/',
    youtube: 'https://www.youtube.com/simplex',
    facebook: 'https://www.facebook.com/simplex',
    linkedin: 'https://www.linkedin.com/company/simplex',
  };

  /**
   * Make request to Simplex API
   */
  private async makeRequest(endpoint: string, params: Record<string, any> = {}): Promise<any> {
    ...
    if (!response.ok) {
      throw new Error(`Simplex API error: ${response.status} ${response.statusText}`);
    }

    const result = await response.json();
    if (result.error) {
      throw new Error(`Simplex API error: ${result.error}`);
    }
...
export const SimplexRamp = {
  // Quick access methods
  async getCurrencies(): Promise<any[]> {
    const instance = new SimplexRampClass();
  ...
  // Static properties
  info: SimplexRampClass.info,
  socialMedia: SimplexRampClass.socialMedia,
};

// Export the class as default
export default SimplexRamp;
