// Guardarian Ramp API Integration
// European crypto gateway
...
export class GuardarianRampClass {
  private config: GuardarianConfig;
  private baseUrl: string;

  constructor(config: GuardarianConfig = {}) {
    this.config = config;
    this.baseUrl = config.baseUrl || (config.sandbox ? 'https://api-sandbox.guardarian.com' : 'https://api.guardarian.com');
  }
  ...
  static readonly info = {
    ...
    apiDocs: 'https://guardarian.com/api/',
    status: 'Active',
  ...
  static readonly socialMedia = {
    twitter: 'https://twitter.com/guardarian',
    telegram: 'https://t.me/guardarian',
    reddit: 'https://www.reddit.com/r/guardarian/',
    youtube: 'https://www.youtube.com/guardarian',
    facebook: 'https://www.facebook.com/guardarian',
    linkedin: 'https://www.linkedin.com/company/guardarian',
  };

  /**
   * Make request to Guardarian API
   */
  private async makeRequest(endpoint: string, params: Record<string, any> = {}): Promise<any> {
    ...
    if (!response.ok) {
      throw new Error(`Guardarian API error: ${response.status} ${response.statusText}`);
    }

    const result = await response.json();
    if (result.error) {
      throw new Error(`Guardarian API error: ${result.error}`);
    }
...
export const GuardarianRamp = {
  // Quick access methods
  async getCurrencies(): Promise<any[]> {
    const instance = new GuardarianRampClass();
  ...
  // Static properties
  info: GuardarianRampClass.info,
  socialMedia: GuardarianRampClass.socialMedia,
};

// Export the class as default
export default GuardarianRamp;
