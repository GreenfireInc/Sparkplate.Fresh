...
/**
 * Domestic courier reference rows by ISO 3166-1 alpha-2 for G20 sovereign members in this module.
 * Reuses BRI layered data where that participant list includes the economy; otherwise informational fallback.
 */
export const G20_DOMESTIC_COURIERS = Object.fromEntries(
  G20_SOVEREIGN_MEMBER_ISO_CODES.map((iso2) => [
    iso2,
 ...
 */
export const G20_INSTITUTIONAL_DOMESTIC_COURIERS = {
  /** Brussels-based EU institutions context (informational). */
  EU: fallbackDomesticCouriers('BE'),
  /** Addis Ababa AU Commission context (informational). */
  AFRICAN_UNION: fallbackDomesticCouriers('ET'),
} as const
