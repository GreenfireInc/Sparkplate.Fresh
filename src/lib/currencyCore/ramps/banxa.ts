...
export class BanxaRampClass {
  private config: BanxaConfig;
  private baseUrl: string;

  constructor(config: BanxaConfig = {}) {
    this.config = config;
    this.baseUrl = config.baseUrl || (config.sandbox ? 'https://api-sandbox.banxa.com' : 'https://api.banxa.com');
  }
  ...
  static readonly info = {
    ...
    apiDocs: 'https://docs.banxa.com/',
    status: 'Active',
    supportedCurrencies: '50+',
    features: ['Credit Card', 'Bank Transfer', 'SEPA', 'Wire Transfer', 'Licensed'],
  };

  // Social Media Links
  static readonly socialMedia = {
    twitter: 'https://twitter.com/banxa',
    telegram: 'https://t.me/banxa',
    reddit: 'https://www.reddit.com/r/banxa/',
    youtube: 'https://www.youtube.com/banxa',
    facebook: 'https://www.facebook.com/banxa',
    linkedin: 'https://www.linkedin.com/company/banxa',
  };

  /**
   * Make request to Banxa API
   */
  private async makeRequest(endpoint: string, params: Record<string, any> = {}): Promise<any> {
    ...
    if (!response.ok) {
      throw new Error(`Banxa API error: ${response.status} ${response.statusText}`);
    }

    const result = await response.json();
    if (result.error) {
      throw new Error(`Banxa API error: ${result.error}`);
    }
...
export const BanxaRamp = {
  // Quick access methods
  async getCurrencies(): Promise<any[]> {
    const instance = new BanxaRampClass();
  ...
  // Static properties
  info: BanxaRampClass.info,
  socialMedia: BanxaRampClass.socialMedia,
};

// Export the class as default
export default BanxaRamp;
