// Knowhere Classic Marketplace Information
// Secondary NFT marketplace on Terra Classic blockchain
// Source: Research compiled from multiple sources

export interface KnowhereClassicListing {
  tokenId: string;
...
export interface KnowhereClassicCollection {
  collectionAddress: string;
  name: string;
  description?: string;
  creator: string;
...
export interface KnowhereClassicPricing {
  tokenId: string;
  collectionAddress: string;
  currentPrice?: number;
  floorPrice?: number;
...
export const knowhereClassicMarketplace = {
  name: "Knowhere (Terra Classic)",
  blockchain: "Terra Classic",
  type: "NFT Marketplace",
  description: "Secondary NFT marketplace for Terra Classic ecosystem. Enables trading of CW721 NFTs on Terra Classic blockchain with marketplace tools and analytics.",
  ...
  api: {
    endpoints: {
      mainnet: "https://api.knowhere.art/v1/classic/",
      classic: "https://api.knowhere.art/v1/classic/",
      base: "https://api.knowhere.art/v1/classic/",
    },
    pricing: {
...
async function getNFTPricing(collectionAddress: string, tokenId: string) {
  const response = await fetch(\`https://api.knowhere.art/v1/classic/nft/\${collectionAddress}/\${tokenId}/price\`);
  const pricing: KnowhereClassicPricing = await response.json();
  return pricing;
}

// Get collection floor price
async function getCollectionFloorPrice(collectionAddress: string) {
  const response = await fetch(\`https://api.knowhere.art/v1/classic/collections/\${collectionAddress}/floor-price\`);
...
async function searchNFTs(keyword: string) {
  const response = await fetch(\`https://api.knowhere.art/v1/classic/nfts/search?keyword=\${encodeURIComponent(keyword)}\`);
  const data = await response.json();
  return data.results;
}
    `,
  ...
  notes: [
    "Secondary NFT marketplace for Terra Classic ecosystem",
...
export default knowhereClassicMarketplace;
