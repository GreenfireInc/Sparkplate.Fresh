 ...
 * 
 * Handles Terra Station browser extension wallet connection for Terra.
 * Terra Station is the native wallet for Terra blockchain.
 */

// Terra Station API types
interface TerraStationWindow extends Window {
  terra?: {
...
declare const window: TerraStationWindow;

// Terra Network configurations
const terraNetworks = {
  mainnet: {
    chainId: 'phoenix-1',
    name: 'Terra Mainnet',
  ...
  testnet: {
    chainId: 'pisco-1',
    name: 'Terra Testnet',
    lcd: 'https://terra-testnet-lcd.publicnode.com'
  }
};

class TerraStationConnect {
  ...
  /**
   * Set up event handlers for Terra Station
   */
  setHooks(modal: any, gtag: any, dispatch: any) {
    ...
    window.terra.on('accountsChanged', (accounts: Array<{ address: string }>) => {
      ...
      dispatch('terraStationAccountsChanged', { accounts });
    });

    // Handle chain changes
    window.terra.on('chainChanged', (chainId: string) => {
      this.chainId = chainId;
      dispatch('terraStationChainChanged', { chainId });
    });
  }

  /**
   * Connect to Terra Station wallet
   */
  async connect() {
    if (!this.isAvailable()) {
      throw new Error('Terra Station extension not installed');
    ...
    try {
      ...
      if (result && result.address) {
        ...
        this.chainId = 'phoenix-1'; // Default to mainnet
  ...
  /**
   * Disconnect from Terra Station wallet
   */
  async disconnect() {
    if (this.isAvailable() && this.connectedAddress) {
      try {
        ...
        console.error('Error disconnecting Terra Station:', error);
      }
    }
    this.connectedAddress = null;
    this.chainId = null;
  ...
  async getChainId(): Promise<string | null> {
    ...
    try {
      // Terra Station doesn't have a direct chainId method
      // We'll use the connected address to infer mainnet
      if (this.connectedAddress) {
        return this.chainId || 'phoenix-1';
      }
      return null;
    } catch (error) {
      return null;
...
export default new TerraStationConnect();
