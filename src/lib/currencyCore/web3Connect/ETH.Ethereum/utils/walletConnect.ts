/**
 * Ethereum WalletConnect Integration
 * 
 * Handles WalletConnect v2 protocol for Ethereum dApp connections.
 * Uses EIP-155 namespace for EVM compatibility.
 */

import { Core } from '@walletconnect/core';
import { Web3Wallet } from '@walletconnect/web3wallet';
import { buildApprovedNamespaces, getSdkError } from '@walletconnect/utils';
import { ethMethods } from './ethRPCMethods';

// App metadata for WalletConnect
const appMetadata = {
  name: 'LoginStandard',
  description: 'LoginStandard - Ethereum Wallet',
...
class ETHWalletConnect {
  ...
  constructor() {
    this.requestTypes = {
      ...
      typedRequests: ['eth_signTypedData', 'eth_signTypedData_v4'],
      signMessageRequests: ['eth_sign', 'personal_sign'],
      sessionRequest: 'session_request',
      sessionProposal: 'session_proposal',
      otherRequests: ['eth_sendRawTransaction', 'wallet_switchEthereumChain', 'wallet_addEthereumChain']
    };
  }
  ...
  async handleSessionProposal({ 
    ...
    // Build EIP-155 accounts for Ethereum
    const ethChains = [
      'eip155:1',        // Ethereum Mainnet
      'eip155:11155111', // Sepolia Testnet
      'eip155:5',        // Goerli Testnet
      'eip155:17000'     // Holesky Testnet
    ];
    ...
    const approvedNamespaces = buildApprovedNamespaces({
      ...
      supportedNamespaces: {
        eip155: {
          chains: ethChains,
          methods: Object.keys(ethMethods),
  ...
  async handleSessionRequest({ 
    ...
    if (!approved) {
      ...
      if (chainIdNum === 1) network = 'mainnet';
      else if (chainIdNum === 11155111) network = 'sepolia';
      else if (chainIdNum === 5) network = 'goerli';
      else if (chainIdNum === 17000) network = 'holesky';
      
      // Use native eth_* methods (no mapping needed)
      const result = await ethMethods[method as keyof typeof ethMethods]({
        wallet,
        params,
        network,
  ...
  getAddressFromRequestParams(method: string, params: any[]): string | undefined {
    ...
    if (requestTypes.transactionRequests.includes(method)) {
      ...
      return method === 'personal_sign' ? params[1] : params[0];
    }
  }
}

export default new ETHWalletConnect();
