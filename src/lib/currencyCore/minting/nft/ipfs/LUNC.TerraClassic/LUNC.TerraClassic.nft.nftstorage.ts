// Terra Classic NFT Minting via NFT.Storage
...
export class TerraClassicNFTStorageMinter {
  private client: NFTStorage;
  private nftStorageGateway = 'https://nftstorage.link/ipfs';
  private rpcEndpoint: string;
  private network: 'mainnet' | 'testnet';

  constructor(nftStorageConfig: NFTStorageConfig, rpcEndpoint: string = 'https://terra-classic-lcd.publicnode.com', network: 'mainnet' | 'testnet' = 'mainnet') {
  ...
  async mintNFT(mnemonic: string, contractAddress: string, fileData: Buffer, fileName: string, metadata: Omit<NFTMetadata, 'image'>, tokenId: string): Promise<MintResult> {
    ...
    const terraClassicNFT = await this.mintTerraClassicNFT(mnemonic, contractAddress, `ipfs://${nftPackage.metadataCid}`, { ...metadata, image: `ipfs://${nftPackage.imageCid}` } as NFTMetadata, tokenId);
    const explorerBase = this.network === 'mainnet' ? 'https://finder.terra.money/classic/tx' : 'https://finder.terra.money/testnet/tx';
    return { ipfsCid: nftPackage.imageCid, ipfsUrl: `${this.nftStorageGateway}/${nftPackage.imageCid}`, metadataCid: nftPackage.metadataCid, metadataUrl: nftPackage.url, tokenId: terraClassicNFT.tokenId, contractAddress, txHash: terraClassicNFT.txHash, finderUrl: `${explorerBase}/${terraClassicNFT.txHash}` };
  }
...
export default TerraClassicNFTStorageMinter;
