<template>
  <div class="mnp-adv-tab-general">
    <!-- Input Section -->
    <div class="space-y-2 px-2">
      <Label for="mnp-adv-seed-input" class="text-sm font-medium text-gray-700 dark:text-gray-300">
        Original Seed Phrase (12 words)
      </Label>
      <textarea
        id="mnp-adv-seed-input"
    ...
    <div class="space-y-2 px-2">
      <Label id="mnp-adv-method-label" class="text-sm font-semibold text-gray-700 dark:text-gray-300">
        Expansion Method
      </Label>
      <div
        class="grid grid-cols-1 lg:grid-cols-2 gap-3"
        role="radiogroup"
        aria-labelledby="mnp-adv-method-label"
      >
        <div
          ...
          role="radio"
          tabindex="0"
          :aria-checked="expansionMethod === method"
          @click="expansionMethod = method as ExpansionMethod"
          @keydown.enter.prevent="expansionMethod = method as ExpansionMethod"
          @keydown.space.prevent="expansionMethod = method as ExpansionMethod"
          :class="[
            'flex items-start gap-3 p-3 rounded-lg border cursor-pointer transition-colors outline-none',
        ...
        >
          ...
          <div class="flex-1 space-y-1">
            <span class="font-semibold cursor-pointer text-sm text-gray-900 dark:text-white">
              {{ info.title }}
            </span>
    ...
    <div class="px-2">
      <button
        ...
        class="w-full px-4 py-3 bg-linear-to-r from-blue-600 to-purple-600 hover:opacity-90 text-white font-medium rounded-lg transition-opacity shadow-md disabled:opacity-50 disabled:cursor-not-allowed"
      >
    ...
    <div v-if="expandedPhrase" class="space-y-3 p-4 bg-green-50 dark:bg-green-900/30 border-2 border-green-500 dark:border-green-400 rounded-lg mx-2">
      <div class="flex items-center gap-2">
        ...
        <Label
          for="mnp-adv-expanded-output"
          class="text-sm font-semibold text-green-900 dark:text-green-100 m-0"
        >
      ...
      <div class="bg-white/50 dark:bg-gray-800/50 p-2.5 rounded-lg border border-green-500/30 dark:border-green-400/30">
        <div class="space-y-1 text-xs font-mono">
          <div class="flex items-center gap-2">
            <CheckCircle2 class="h-3 w-3 text-green-600 dark:text-green-400 shrink-0" aria-hidden="true" />
